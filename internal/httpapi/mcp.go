...
func (s *Server) mcp(w http.ResponseWriter, r *http.Request) {
	...
	case "tools/list":
		...
		tools := []any{
			...
			map[string]any{"name": "analyze_attribution", "description": "전환을 채널 그룹에 배분합니다. model은 last_non_direct, first_touch, last_touch를 지원하고 lookback_days는 기본 30일입니다.", "inputSchema": map[string]any{"type": "object", "properties": map[string]any{"site_id": map[string]string{"type": "string"}, "environment": map[string]any{"type": "string", "default": "prd"}, "model": map[string]any{"type": "string", "enum": []string{"last_non_direct", "first_touch", "last_touch"}}, "lookback_days": map[string]any{"type": "integer", "default": 30}, "from": map[string]string{"type": "string"}, "to": map[string]string{"type": "string"}}, "required": []string{"site_id", "from", "to"}}},
...
func (s *Server) mcpCall(w http.ResponseWriter, r *http.Request, req rpcRequest) {
	...
	case "analyze_attribution":
		model := stringArgDefault(call.Arguments, "model", "last_non_direct")
		if _, ok := insight.AttributionOrder(model); !ok {
			writeJSON(w, 200, rpcResult(req.ID, mcpText("model must be first_touch, last_touch or last_non_direct", true)))
			return
		}
		lookback := 30
		if value, ok := call.Arguments["lookback_days"].(float64); ok {
			lookback = int(value)
		}
		report, err := insight.New(s.DB).Attribution(r.Context(), siteID, stringArgDefault(call.Arguments, "environment", "prd"), from, to, lookback, model)
		if err != nil {
			writeJSON(w, 200, rpcResult(req.ID, mcpText(err.Error(), true)))
			return
		}
