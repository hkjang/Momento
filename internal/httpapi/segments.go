...
// The compiler moved to internal/segment so the scheduled delivery can evaluate
// the same definition the screens do. These names keep the call sites reading
// the way they did.
type segmentNode = segment.Node

type dimensionResolver = segment.Resolver

func compileSegment(node segmentNode, resolver dimensionResolver, alias string, args *[]any, depth int) (string, error) {
	return segment.Compile(node, resolver, alias, args, depth)
}

func (s *Server) newDimensionResolver(ctx context.Context, siteID uuid.UUID, environment string) (dimensionResolver, error) {
	return segment.NewResolver(ctx, s.DB, siteID, environment)
}
...
func (s *Server) saveDimension(w http.ResponseWriter, r *http.Request) {
	...
	if !segment.PropertyKeyPattern.MatchString(in.Name) || !segment.PropertyKeyPattern.MatchString(in.PropertyKey) {
