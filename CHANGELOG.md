...
- **JSON이 아닌 200이 "데이터 없음"으로 도착하고 있었습니다.** 콘솔의 모든 요청이 지나가는 `api()`가 본문을 `response.json().catch(() => ({}))`로 읽고 그것을 돌려줬습니다 — **JSON이 아닌 것을 실은 200은 필드가 없는 객체**가 되고, 거기서 읽은 모든 숫자가 `undefined`가 되고, 화면은 **이벤트를 한 번도 보내지 않은 사이트에 그리는 그 빈 상태**를 그렸습니다. 프록시의 오류 페이지, 게이트웨이의 점검 페이지, 앞단의 로그인 리디렉션이 전부 **200에 HTML**을 실어 답하고, **온프레미스가 바로 서비스 앞에 무언가가 서 있는 곳**입니다.
- 이제 오류이고 화면이 설명합니다: *"요청이 Momento가 아닌 무언가에 닿았습니다 … **데이터가 없는 것이 아니라 답이 도착하지 않은 것입니다**"*. 다시 시도를 제안합니다 — 앞단의 로그인 리디렉션은 다른 곳에서 로그인하면 풀리기 때문입니다.
- **파싱과 판정을 분리했습니다**: 실패한 요청의 본문도 JSON이 아닐 수 있고 그때는 **상태 코드가 유일한 정보**이므로, 그것을 "JSON이 아님"으로 덮어쓰지 않고 502를 502라고 말합니다.
- **이 파일에는 테스트가 하나도 없었습니다.** 이제 넷입니다: 파싱 안 되는 성공은 거부, 정상·빈 성공은 그대로 통과, 서버가 준 오류 코드와 메시지가 화면까지 살아 도착, 본문이 JSON이 아닌 오류는 여전히 502.

## v0.34.23

- **네트워크가 잠깐 끊기면 멀쩡한 세션이 로그아웃됐습니다.** 세션 확인이 **어떤 실패든** "로그인 안 됨"으로 다뤄서 — 네트워크 끊김, 500, 타임아웃 전부 — 사용자를 비우고 **로그인 폼**을 보여 줬습니다. 다시 로그인해도 같은 이유로 실패하고 화면은 **왜인지 말하지 않습니다.** 멀쩡한 세션을 가진 사람이 로그아웃되고 **해결책은 비밀번호를 다시 치는 것**이라고 안내받았습니다.
- **서버가 거부한 것(401·403)만이 그 질문에 대한 답입니다.** 그 밖은 알아내지 못한 것이고, 이제 둘은 화면에 다르게 도착합니다: 거부는 로그인 폼, 확인 실패는 *"로그아웃된 것이 아니라 서버에 닿지 못한 것입니다"* 와 오류 내용, **다시 확인** 버튼.
- **로그아웃은 서버 응답과 무관하게 로컬 상태를 비웁니다** — 나가겠다고 한 사람은 나간 것이고, 요청 실패가 그를 로그인된 것처럼 남겨 둬서는 안 됩니다.
- 한 컨텍스트에서 둘, 지난 릴리스에 `SiteContext`에서 둘. **넷의 공통점은 어디에 있는가입니다** — 셸의 컨텍스트는 **React Query 밖**에 있어서 이 콘솔이 실패한 읽기를 보고하려고 갖고 있는 **어떤 규칙도 닿지 않고**, 모든 화면이 그 위에 서 있습니다.

## v0.34.22

- **늦게 도착한 이전 사이트의 응답이 이길 수 있었습니다.** 사이트의 환경 목록을 읽는 effect에 **취소가 없어서**, 사이트를 빠르게 바꾸면 두 읽기가 동시에 날고 느린 쪽이 이전 사이트의 것이면 **나중에 도착해 이깁니다.** 선택기는 **이전 사이트의 환경 목록을 새 사이트 이름 아래** 제시하고, 거기서 고른 값이 **새 사이트에는 없을 수도 있는 채로** 모든 화면에 전달됩니다 — 지난 릴리스의 조회 키 문제와 같은 모양이 **한 층 아래에서**. 요청은 옳았고, 읽는 사람이 보게 된 것은 아니었습니다.
- **그리고 선택된 환경에 의존하면서 그 환경을 설정하고 있었습니다** — prd와 stg를 오갈 때마다 목록을 다시 가져와 아무것도 새로 배우지 않았습니다. 이제 현재 값을 **ref로 읽습니다**: effect가 필요로 하지만 다시 시작할 이유는 아닌 입력.
- 테스트가 effect의 **의존성 배열을 직접 읽어** 사이트 하나뿐일 것과 **정리 함수가 있을 것**을 요구합니다. 환경을 되돌리면 거기 무엇이 들어 있는지 대며, 정리 함수를 지우면 어느 응답이 이기는지 말하며 실패합니다.

## v0.34.21

- **"사이트가 없습니다"가 "목록을 못 읽었습니다"와 같은 말이었습니다.** `AppShell`은 *"모든 화면이 React Query로 읽으므로 구독 하나가 전부를 덮는다"* 고 적어 뒀고 화면들에 대해서는 참인데, **셸 자신의 부팅은 화면이 아닙니다.** `SiteContext`는 사이트·환경 목록을 React Query 밖에서 **catch 없이** 읽었습니다. 사이트 목록이 실패하면 `site`가 null이 되어 그것을 가드로 쓰는 **화면 열다섯 개가 전부** *"분석할 사이트가 없습니다 — 첫 사이트를 만들고 Tracking SDK를 설치해 보세요"* 를 **사이트 만들기 버튼과 함께** 그렸습니다.
- 세션이 만료됐거나 네트워크가 끊긴 사람은 사이트가 없다는 말을 듣고 **이미 가진 사이트의 중복본을 만들어 고치라는 권유**를 받았습니다 — 오류 메시지보다 나쁜, **행동이 딸려 오는 오답**입니다. 이제 실패를 기록하고 `NoSite`가 둘을 구분해, **열다섯 화면 전부가 어느 것도 고치지 않고** 그 구분을 갖습니다. 환경 목록도 잡습니다 — 사이트에는 환경이 최소 하나 있으므로 **못 읽은 것은 실패이지 빈 답이 아닙니다.**
- **테스트 첫 판본이 주입된 결함을 통과했습니다.** `States.tsx`에서 문자열 `loadError`만 찾았는데 `const loadError = null`에도 그게 있어, 컨텍스트 읽기를 상수로 바꿔도 초록이었습니다. 이제 그 값이 **`useSite()`에서 오는지**를 봅니다.

## v0.34.20

- **화면 넷이 환경 전환을 무시하고 있었습니다.** `keepPrevious.ts`는 *"빈 화면보다 나쁜 단 하나는 한 사이트의 숫자가 다른 사이트 이름 아래 표시되는 것 … 모든 분석 조회 키는 둘 다 담습니다"* 라고 적어 뒀는데 넷은 아니었고, 결과는 그 문장이 막으려던 것보다 **나쁩니다**. `rangeQuery`가 환경을 URL에 넣으므로 **요청은 옳았지만**, React Query는 **가져올지 말지를 키로 판단**합니다 — 키가 안 바뀌면 **아무 요청도 일어나지 않고**, 화면은 **방금 떠난 환경의 캐시**로 **새 환경 이름 아래** 답합니다.
- 이커머스·사내 사용 현황·방문자 검색·방문자 추적. 프로덕션에서 스테이징으로 바꾸면 **툴바 칩만 바뀌었습니다.** 그 숫자들은 읽는 사람이 알아챌 방식으로 낡지 않았습니다 — **이제 아무도 묻지 않는 질문에 대한 진짜 답**이었으니까요. 나머지 열아홉 개는 이미 옳았고, 그래서 읽어서는 알아채기 어려웠습니다.
- **규칙은 목록이 아니라 소스에서 읽습니다**: *URL을 `rangeQuery`로 만드는 조회는 답이 환경에 따라 달라지므로 키가 그렇게 말해야 한다.* 테스트가 `useQuery` 블록을 직접 찾고 **스무 개 미만이면 통과시키지 않습니다**(매처가 망가지면 "전부 올바르다"고 보고하는 대신 실패해야 하므로).

## v0.34.19

- **같은 사이트가 식별자가 들어온 문에 따라 다른 답을 받고 있었습니다.** `identify()` 주석은 *"수집기의 개인정보 필터는 이 값을 보지 않으므로 여기가 유일하게 결정하는 자리"* 라고 했는데, v0.34.0부터 사실이 아닙니다 — 서버 대 서버 수집은 브라우저를 거치지 않고, 그 점이 "트래커가 유일한 자리"인 것을 설계가 아니라 결함으로 만들었습니다. 주석을 고치자 나머지가 나왔습니다: 두 반쪽은 **두 언어로 쓰인 하나의 규칙**이고 트래커는 둘이 "같은 것을 거부한다"고 주장하는데, **아니었습니다.** 트래커는 식별자 속 **긴 숫자 나열**을 거부하고 수집기는 탐지하지 않아, 브라우저는 `user-9988776655`를 보낼 수 없고 **서버 대 서버 통합은 보낼 수 있었습니다.**
- 수집기가 이제 그 규칙을 적용하되 **사용자 ID에만** 적용합니다. `maskPIIString`에는 **의도적으로 넣지 않았습니다** — 그건 모든 속성을 훑고, 주문 번호·거래 ID·타임스탬프가 전부 긴 숫자 나열이라 **아무것도 보호하지 않으면서 평범한 데이터를 파괴**하게 됩니다. 사용자 ID는 **그 값이 사람이 되는 값**이라 다릅니다.
- 형태들은 `testdata/pii_identifiers.json`에 있고 **양쪽 테스트가 같은 파일을 읽습니다** — 한쪽에 추가한 예시는 다른 쪽이 통과해야 하는 예시가 됩니다. 파일은 **양면을 담습니다**: 거부할 식별자와, `identify()`가 담으라고 있는 것들(사번·불투명 ID·계정명). 전부 거부하는 규칙은 아무것도 보호하지 않으면서 한쪽만 보는 테스트를 통과하기 때문입니다.

## v0.34.18

- **나란히 놓인 두 전환율에 설명이 없었습니다.** `metricMeaning`은 자기 목적을 *"서로 헷갈릴 수 있는 지표 이름을 설명한다"* 고 적어 뒀는데, 쿼리 빌더는 `user_conversion_rate`와 `session_conversion_rate`를 나란히 제공하면서 **둘 다 설명하지 않았습니다.** 한 단어만 다른 두 이름에 **서로 다른 분모** — 두 번 전환한 사람은 앞에서는 1, 뒤에서는 2입니다. 이 모듈이 존재하는 정확히 그 경우이고, 고르는 사람에게는 이름 말고 아무것도 없었습니다. 이제 둘 다 **무엇을 무엇으로 나누는지** 말합니다.
- **규칙은 기억해야 할 두 번째 목록이 아니라 제공 목록에서 끌어냅니다**: *제공되는 다른 지표와 단어를 나눠 쓰는 지표는 설명이 있어야 한다.* 테스트가 쿼리 빌더의 `metrics` 배열을 직접 읽어 해당하는 것들을 계산하므로, **기존 지표 옆에 새 지표를 추가하면 이미 규칙에 걸립니다.**
- 함께 확인하고 참이었던 것: `sessions`/`sessions_started`가 *"어느 쪽이 어느 쪽인지 말하는 이름으로 둘 다 존재한다"* — 둘 다 제공되고 설명도 있습니다. `compareFunnelSeries`는 모든 Segment를 baseline과 비교하고 **20명 미만 Cohort는 거부**합니다.

## v0.34.17

- **"보존이 한 번도 안 돌았다"와 "읽기가 실패했다"가 같은 답이었습니다.** `retentionRun`은 자기에 대해 *"정책만으로는 운영자가 요청한 것을 알 뿐 실제로 일어나고 있는지는 알 수 없었습니다 — 한 달째 실패하는 작업이 할 일 없는 작업과 똑같아 보였습니다"* 라고 적어 뒀고, 기록 자체는 실패도 남깁니다. 그런데 **읽는 쪽이 어떤 에러에서도 `nil`을 돌려줬고**, `nil`은 한 번도 안 돌았을 때 화면이 보여 주는 것입니다 — **운영자가 "뭔가 일어나고 있나"를 물으러 여는 화면이** "아무 일도 없었습니다"라고 답했습니다. 그 기록이 없애려던 모호함이 다른 문으로 들어온 것입니다.
- **Workspace 집계가 존재하는 이유는 한 번도 테스트된 적이 없었습니다.** *"두 서비스에서 보인 사람은 여기서 한 사람"* 이라는 주장은 참이고 응답도 중복 제거 인원과 서비스별 합을 나란히 싣지만, 픽스처는 자기를 *"두 서비스, 양쪽에서 활동하는 SSO 사용자"* 라고 설명하면서 **두 번째 서비스에 이벤트가 하나도 없습니다.** 이제 테스트가 그 사람을 실제로 만듭니다(수집기를 통해 전송) — **Workspace 5명, 서비스별 합 6명.** 서비스 행들이 옆에 적힌 합과 더해지는지도 확인하고, 고유 인원을 서비스별로 세게 만들면 실패합니다.

## v0.34.16

- **정책 화면이 한도가 아닌 것을 한도로 보여 주고 있었습니다.** Query Policy 엔드포인트는 다섯 한도와 함께 `defaults`(저장된 정책이 있는지 여부를 말하는 **불리언**)를 답하는데, 화면은 응답 **전체를 폼에 캐스팅**했습니다(`policy.data as typeof policyForm`) — TypeScript는 받아들이고 브라우저는 신경 쓰지 않습니다. 폼은 자기가 가진 키마다 컨트롤을 그리므로, 관리자는 **여섯 번째 숫자 입력란**을 봤습니다: 라벨 맵에 항목이 없어 **원본 키 `defaults`** 가 라벨이 되고 **숫자 입력란에 불리언**이 앉은 채로. 이건 **어떤 한도가 적용 중인지 알려 주는 화면**입니다.
- 이제 다섯 개를 **이름으로** 가져오고, 응답이 빠뜨린 값은 **화면에 있는 값을 유지**하며(부분 응답이 보고 있는 컨트롤을 비우지 않도록), **유한한 숫자가 아닌 값은 받아들이지 않습니다.** 이번 조사를 시작하게 한 주석의 주장 — 가드와 관리 화면이 같은 정의를 읽는다 — 은 **참이었습니다.**
- **적어 두되 고쳤다고 하지 않는 것**: 세션 중 전체 스위트 한 번이 **부하 하네스의 지연 표를 출력하며** 실패했는데, 그 하네스는 opt-in이고 그 명령에서 켜져 있지 않았습니다. 이후 전체 실행 세 번(둘은 캐시 무시)이 통과했습니다. **재현하지 못했고 그것을 이유로 아무것도 바꾸지 않았습니다.**

## v0.34.15

- **콘솔이 사이트 정책이 거부할 기간을 제시하고 있었습니다.** 서버는 `max_exact_days`를 보내며 *"콘솔은 이 값으로 기간 선택지를 만들므로 정책이 거부할 범위를 제시하는 일이 없습니다"* 라고 적어 뒀는데, 그건 `RangeSelect`에서만 참이었습니다. `AnalysisToolbar`는 **7·30·90을 하드코딩**했고, 그건 **개요와 방문자 인사이트 화면의 컨트롤**입니다. 14일로 제한된 사이트에서 드롭다운은 `RANGE_EXCEEDS_POLICY`로 답할 기간 두 개를 제시했습니다 — 사람들이 가장 먼저 여는 두 화면에서, **기다린 뒤에야**. 관리자는 데이터베이스를 지키려고 한도를 내리는데 콘솔은 모든 읽는 사람에게 그것을 넘으라고 권하고 있었습니다.
- **기간을 제시하는 컨트롤이 한도 적용 여부를 스스로 정할 수는 없습니다.** 테스트가 소스를 읽어 모든 기간 컨트롤이 `allowedRanges`로 거르는지, 그것을 붙이는 모든 화면이 사이트 한도를 넘기는지 확인하고, 마운트된 컨트롤을 **8개 미만으로 찾으면 통과시키지 않습니다.**
- **사이트 관리 라우트를 한 곳의 멤버십 검사에 묶었습니다.** `resolveSiteByID`가 존재하는 이유는 여섯 관리 핸들러가 각자 정하다 **넷이 틀리게 정했기** 때문입니다 — 한 Workspace에만 속한 `workspace_admin`이 다른 Workspace의 사이트를 이름 바꾸고, 키를 회전시키고, **삭제할 수 있었습니다.** 미들웨어는 호출자가 **어딘가의** `workspace_admin`인지만 검사합니다. 테스트는 핸들러가 아니라 **라우트를 읽습니다** — 그 속성은 호출자가 준 uuid로 주소 지정된다는 것에 속하기 때문입니다.

## v0.34.14

- **Segment의 행동 기반 집계가 한 문장 안에서 두 번 실행되고 있었습니다.** `entity.sessions >= 3` 같은 조건은 `entity_id IN (SELECT ... GROUP BY entity_id HAVING ...)`로 컴파일되고, 그 서브쿼리는 **상관되어 있지 않습니다** — 필터링 중인 행을 참조하는 것이 없습니다. 그런데 Cohort 그리드는 Segment 조건을 **두 번** 적용합니다(Cohort 정의에 한 번, 재방문 활동에 한 번 — 세지도 않은 사람을 유지시키면 안 되므로). 그래서 같은 서브쿼리가 두 번 나타나고 PostgreSQL은 **사이트 전체 이력에 대해 두 번 실행**했습니다.
- `CompileHoisted`가 각 집계를 **이름 붙은 CTE로 끌어올리고** 그 자리에 참조를 남깁니다. **이것들만 움직입니다** — `event.has`는 바깥 행에 대한 상관 EXISTS라 제자리에 남습니다. **cohort-90d+segment 6.838s → 5.410s**, cohort 365d+segment 6.979s → 5.869s.
- **"같은 조건"은 SQL에 대한 주장이므로 같은 데이터에 양쪽을 실행해 누가 나오는지 비교합니다.** 다섯 정의는 부주의한 hoist가 어긋날 자리들입니다: 집계 단독, 이벤트 수준 조건과 AND, 이벤트 수준 조건과 OR, 집계 둘, 중첩 그룹. **양쪽 모두 누군가를 골라야 합니다** — 빈 집합 둘은 어느 쪽이 틀렸든 일치하므로. 실제로 "집계 둘"의 첫 판본이 아무도 고르지 않아 둘 다 매칭되는 쌍으로 바꿨습니다.
- `Compile`은 조건이 한 번만 나타나는 호출부를 위해 기존 동작을 그대로 유지합니다 — 거기서는 CTE가 아무것도 벌어 주지 않습니다.

## v0.34.13

- **Cohort 그리드가 한 분기를 묻는데 사이트 전체를 묶고 있었습니다.** 사람의 Cohort은 첫 활동이 떨어지는 구간이고 그건 전체 이력이 필요하므로, 그리드는 **수집한 모든 이벤트를 묶어** 답했습니다 — 200만 이벤트에서 그 묶기는 **130MB를 디스크로 정렬**했습니다. 일별 방문자 롤업이 이미 답을 갖고 있고(`insight.FirstSeenCTE`, 개요의 신규 사용자 수가 읽는 정의), **둘 중 더 진실한 쪽**이기도 합니다 — Raw Event는 만료되지만 롤업은 유지되므로 초기 이벤트가 만료된 사람을 이벤트 판본은 **다시 신규라고 불렀습니다.** 같은 행에 **쿼리 2.59s → 1.93s**, API 경유 2.456s → 2.164s.
- **두 정의가 어긋날 수 없게 했습니다.** Cohort 이벤트나 Segment는 어떤 이벤트가 Cohort을 정의하는지 좁히므로 그 경우는 이벤트를 계속 읽습니다 — 즉 "처음 본 시점"의 정의가 둘 남고, **오늘 일치하는 둘은 어긋날 수 없는 둘과 다릅니다.** 테스트가 같은 기간에 그리드를 양쪽 경로로 실행해(모집단이 동일하도록) **셀 단위로 같은 답**을 요구합니다. 롤업 읽기에서 신원 조인을 빼면 `cohort 0 size: the rollup says 3 and the events say 2`로 실패합니다.
- **실패한 조회가 "그런 사람 없음"으로 읽히고 있었습니다.** `resolveTraceSubject`가 존재 확인 두 개를 버려서 **데이터베이스 실패가 "찾지 못함"** 이 되었고, 누군가를 찾던 관리자는 그 사람이 존재하지 않는다는 답을 받았습니다. 첫 조회의 어떤 에러든 "행 없음"으로 다루던 것도 고쳤습니다 — 그 뜻인 것은 `pgx.ErrNoRows` 하나뿐입니다.
- **재지만 싣지 않은 것**: 행동 기반 집계는 `entity_id IN (서브쿼리)`로 컴파일되는데 `entity_id`는 뷰가 계산하는 값이라 통계가 없어 PostgreSQL이 **66억 행으로 추정**하고 정렬을 고릅니다. 멤버십을 materialized CTE로 끌어올려 조인하면 **6.9s → 5.2s**로 일관되게 빨라집니다. 싣지 않은 이유는 Segment 컴파일러가 집계를 **이름 붙은 CTE로 내보내게** 해야 하고, 부주의하면 **이벤트 수준과 사람 수준이 섞인 조건이 고르는 대상이 바뀌기** 때문입니다.

## v0.34.12

- **데이터 품질 화면의 수집 적체 수치는 한 번도 동작한 적이 없습니다.** 쿼리가 `event_inbox.received_at`을 읽는데 **그런 컬럼은 없습니다**(그 테이블에는 `created_at`이 있습니다) — 작성된 날부터 모든 요청에서 실패해 왔고, **에러가 버려졌으므로** 화면은 "대기 0건, 지연 0초"를 보고했습니다. 운영자는 그 두 숫자로 수집이 건강한지 판단하는데, 받을 수 있는 답은 **"전부 정상" 하나뿐**이었습니다 — 건강 화면이 기본값으로 절대 주면 안 되는 답입니다. 테스트가 실제 작업을 받은 편지함에 넣고 화면이 그것을 보는지 확인합니다.
- **찾은 방법은 찾아서가 아니라 에러 버리기를 그만둬서입니다.** **도입 모집단**은 화면의 모든 Adoption 비율의 분모라 버려진 실패가 **모든 기능을 아무도 안 쓰는 것처럼** 만듭니다. **실험 변형 수**는 버려지면 사용자 0명·전환 0건이 되고 비교는 그것을 **진 변형**으로 읽습니다. **검색 요약**은 아무도 검색하지 않는 사이트를 보고합니다. 그리고 **진단 화면의 네 읽기**가 가장 나쁩니다 — 그 화면의 모든 검사가 거기서 파생되므로, 실패한 쿼리는 "지난 24시간 이벤트 0건"이 되고 검사는 그것을 **"수집이 안 되고 있습니다"** 로 바꿉니다. 운영자는 **자기 쿼리가 깨진 화면으로부터 트래킹이 깨졌다는 말을 듣고** 트래커를 보러 갑니다.
- 전부 `writeQueryError`를 지나갑니다. **Cardinality 한도만 의도적으로 관대한 읽기를 유지합니다** — 그건 측정이 아니라 설정이고, 행이 없는 환경은 정당하게 설정하지 않은 것입니다.
- **검색 리포트가 세 번 기다리고 있었습니다.** 독립적인 세 읽기(합계·검색어·조치 대상)가 차례로 돌아 페이지가 셋의 합을 기다렸습니다. 이제 한 번에 나갑니다: **4.592s → 2.910s**(90일 4.486s → 2.806s).

## v0.34.11

- **무거운 조회 열 개가 제한 시간 밖에 있었습니다.** `analyticalContext`는 자기를 이렇게 설명합니다 — *"제한이 없으면 10년짜리 범위 하나가 브라우저가 포기할 때까지 연결을 붙잡고, 운영자는 이유 대신 멈춘 페이지를 봅니다. **그래서 모든 무거운 조회는** 제한 시간 아래에서 실행됩니다."* 이벤트·페이지·방문자 목록, **쿼리 빌더**, 검색 리포트, 기능 인텔리전스, Journey 분석 둘, 실험 리포트, 자연어 분석이 전부 `analytics_events`를 **요청 자신의 컨텍스트로** 읽고 있었습니다. 넓힌 범위 하나가 연결을 붙잡고 **탭을 닫는 것만이 그것을 끝냅니다** — 제한 시간이 존재하는 바로 그 실패입니다. 그중 쿼리 빌더가 가장 나쁩니다: 기간·차원·Segment를 읽는 사람이 **자유롭게 고르는** 유일한 화면입니다.
- 열 개 모두 제한 시간 아래로 들어갔고 실패는 `writeQueryError`를 지나갑니다 — 시간이 다 된 조회는 드라이버 메시지를 실은 맨 500이 아니라 **안내가 붙은 `QUERY_TIMEOUT`** 으로 답합니다.
- **테스트가 그 주장을 붙잡습니다.** 소스를 읽습니다 — 행동 기반으로 쓰려면 쿼리를 일부러 느리게 만들어야 하고 그건 규칙이 아니라 픽스처를 고정합니다. `analytics_events`를 읽는 핸들러를 15개 미만으로 찾으면 **실행을 거부합니다**(정규식이 매칭을 멈추면 "전부 묶여 있다"고 보고하는 대신 실패해야 하므로). 하나를 되돌리면 이름과 줄 번호를 대며 실패합니다.
- **경험 리포트가 두 번 기다리고 있었습니다.** 사이트 전체 측정과 코호트별 측정이 두 단계였고 **두 번째가 첫 번째에서 필요로 하는 것은 없었습니다** — Segment 3개면 네 개 중 가장 느린 것을 기다린 뒤 다시 네 개 중 가장 느린 것을 기다렸습니다. 이제 같은 `QueryConcurrency` 천장 안에서 한 번에 나갑니다: **7.524s → 5.145s**(90일 7.378s → 4.847s). 더 이상 제품에서 가장 느린 리포트가 아닙니다.

## v0.34.10

- **`GROUP BY` 안의 `count(DISTINCT ...)`도 같은 문제입니다.** 그 집계는 해시로 답할 수 없어 PostgreSQL이 `(그룹, distinct 키)`로 기간 전체를 정렬합니다 — 이벤트 리포트는 **200만 행을 묶어 여섯 줄을 답합니다.** 두 번 묶으면(이벤트와 사람으로, 그다음 이벤트로) 해시 집계 두 번으로 같은 행에 도달합니다: **쿼리 4.06s → 1.57s**(행 동일), **API 경유 4.230s → 1.408s**.
- **접지 않은 것들과 그 이유.** 현재 기간 채널 읽기는 고유 인원과 고유 세션 **두 개의 다른 키**를 세므로 그대로 뒀습니다 — 하나로 접으려면 "세션은 평생 한 사람의 것"을 가정해야 하고, 그건 **누군가 세션 중간에 로그인하기 전까지만** 맞습니다. 사내 사용 현황의 여섯 기준, 기기별 행, 그룹된 오류 목록도 같은 이유입니다. 방문자 목록은 **정렬된 `array_agg`** 가 어차피 정렬을 강제해 얻을 것이 없고, 정렬된 집계 주위를 접는 것은 답을 조용히 바꾸기 좋은 방법이라 그대로 뒀습니다.
- 부하에서 모든 프로브가 더 빠르거나 같고 하네스는 통과합니다.

## v0.34.9

- **`count(DISTINCT ...)`는 해시로 답할 수 없습니다** — PostgreSQL은 그걸 위해 입력 전체를 정렬하고, 200만 이벤트에서 그 정렬은 **디스크로 넘칩니다**(임시 파일 100MB). 같은 데이터베이스·연결·캐시에서 측정: **지표 집합 2.50s → 1.25s**, **인사이트 기간 집계 3.64s → 2.40s**, 단순 고유 인원 수 **2.17s → 0.98s**. 지표 집합은 개요마다 두 번·인사이트 리포트마다 두 번·정기 배달마다 한 번 돕니다. 둘 다 **사람 단위로 먼저 접고** 그룹을 셉니다 — 이벤트 수도 같은 패스에서 나오므로 두 번 읽지 않습니다. 다시 쓴 쿼리가 원본과 **바이트 단위로 같은 행**을 답하는지 적용 전에 확인했습니다. 엔터프라이즈 리포트의 같은 모양 셋도 함께 고쳤습니다.
- **동봉하던 `compose.yml`이 데이터베이스에 `/dev/shm`을 주지 않고 있었습니다.** Docker 기본값은 64MB이고 PostgreSQL은 병렬 질의의 공유 해시 테이블을 거기에 둡니다. 부하 하네스가 `could not resize shared memory segment ... No space left on device`로 **500을 반환**했습니다. 위 재작성이 만든 문제는 아니지만(어떤 병렬 계획도 그럴 수 있습니다) 재작성이 병렬 계획을 흔하게 만들고, 뜻은 정확히 이렇습니다 — **리포트는 커질수록 느려지지 않습니다. 되고, 되고, 되다가, 플래너가 병렬로 가기로 한 날 실패합니다.** `shm_size: 1gb`를 설정하고, 오프라인 가이드는 직접 운영하는 컨테이너에도 같은 값을 주라고 말하며, 테스트가 둘 다 고정합니다.
- **그 실패에 말이 붙습니다.** `shared memory`가 들어간 `53100`은 기간이 넓어서 생긴 실패가 아닌데, 일반 실패로 보고하면 읽는 사람을 **문제가 아닌 기간을 좁히러** 보냅니다. API가 설정 이름을 대고, 콘솔은 **계획이 바뀌는 날 나타나므로 어제 되던 화면이 오늘 실패할 수 있다**고 설명합니다. 진짜 디스크 가득참도 `53100`이라, 그것이 이쪽으로 잘못 분류되지 않는지 테스트가 확인합니다.
- 부하에서 모든 프로브가 더 빠르거나 같고 하네스는 통과합니다. **끝단 수치의 일부는 SQL이 아니라 공유 메모리 때문일 수 있어**(기준선이 64MB 컨테이너였습니다) 쿼리별 수치는 격리해서 측정했습니다.

## v0.34.8

- **Confluence에 JSON 덩어리를 게시하고 있었습니다.** Webhook은 payload를 다른 것에게 넘기고 메일 게이트웨이는 자기가 포맷하지만, **Confluence 페이지는 완성된 결과물**이고 렌더링을 우리가 책임지는 유일한 채널입니다. 이제 페이지입니다: 이번 기간과 이전 기간, 각 지표가 **어느 쪽으로 움직였는지**, 순위 매긴 findings와 각각의 조치, AI 행과 합계, 방문자 인사이트의 headline·KPI. 숫자는 `1.2043e+04`가 아니라 `12,043`으로 인쇄합니다. **원본 문서는 접힌 섹션에 그대로** 둡니다 — 이 페이지를 파싱하는 팀이 잃는 것이 없어야 합니다.
- **페이지에 닿는 모든 값을 이스케이프합니다.** storage format은 XHTML이고 거기 닿는 값은 이벤트 속성 — 사이트가 보낸 무엇이든입니다. 이벤트 속성이 Confluence 매크로가 될 수 없다는 테스트가 있습니다.
- **제일 느린 리포트가 전체 행을 실체화하고 있었습니다.** Segment 비교를 켠 경험 리포트는 15초 예산에 11.9초로 제품에서 가장 느렸는데, 코호트별 쿼리가 `SELECT e.*`로 시작합니다 — 무엇을 세기도 전에 해당 기간 전체가 jsonb 두 덩이를 달고 실체화됩니다. 개요의 지표 집합에서 이미 고쳤던 실수가, **비교 대상마다 한 번씩 도는 쿼리**에 있었습니다. 집계가 읽는 세 컬럼으로 좁혀 200만 이벤트 중앙값 **11.886s → 8.047s**(90일 변형 12.075s → 8.126s)입니다.
- **1년짜리 코호트 프로브 두 개가 아무것도 재지 않고 있었습니다.** 사이트는 질의 정책보다 넓은 기간을 거부하고 기본값은 180일이라, 둘은 **2밀리초에 400을 받고** 있었습니다 — 푸시마다 돌지 않는 opt-in 하네스라 그대로 남았습니다. 1년 코호트는 한도를 올린 관리자가 실제로 돌리는 것이므로 프로브를 좁히는 대신 **하네스가 한도를 올립니다.** 이제 2.607s·6.692s로 실제로 돕니다.
- **가드 하나가 세션 없이 통과하고 있었습니다.** 통합 픽스처의 `session` 헬퍼는 토큰이 고정 문자열이라 해시가 매 실행 같은데 `ON CONFLICT DO NOTHING`이 이미 있던 행을 그대로 뒀습니다. 한 시간 뒤 그 행은 만료되고, 헬퍼는 성공했다고 보고하고, 테스트는 세션과 무관한 요청에서 `invalid session`으로 실패합니다. 저장된 세션을 만료시키고 되돌리면 **같은 줄에서 원래 실패가 재현됩니다.**

## v0.34.7

- **개요 다이제스트가 기간의 절대값만 보내고 있었습니다.** "12,043 users"는 요약이 아닙니다 — 받은 사람은 그게 올해 최고의 주인지 지난주의 절반인지 알 수 없고, 그 이름을 딴 화면은 모든 수치 옆에 지난 기간을 놓습니다. 이제 `current`·`previous`와 **둘 사이의 변화율**을 화면과 같은 읽기에서 함께 싣습니다.
- **`인사이트 요약`에 인사이트가 없었습니다.** 같은 분기를 공유해서 개요와 똑같은 다섯 숫자를 보냈습니다 — 변화도 심각도도 권고도 순위도 없이. 그 화면의 내용 전부가 순위 매긴 findings인데, **리포트가 대신 해 주기로 한 비교를 읽는 사람이 직접** 해야 했습니다. 순위는 `insight.RankInsights`로, 기간 수치는 `insight.Metrics`·`PreviousRange`로 옮겨 양쪽이 같은 정의를 씁니다.
- **`ProcessPending`이 한 배치 100행만 처리하고 있었습니다.** 이름은 "받은 편지함을 비운다"이고 모든 호출부가 그렇게 쓰는데 — 통합 가드들은 배치를 보내고 이걸 부른 뒤 **전부 도착했다는 전제로** 테이블을 읽습니다. 100행을 넘으면 **일부만 도착한 기간을 측정하고 그 빈틈을 자기가 보던 것의 결함으로 보고합니다.** 이 저장소에서 실제로 일어났습니다: 전체 스위트에서 세션 가드가 "방문에 이벤트가 하나뿐"이라며 실패했고 혼자서는 세 번 연속 통과했습니다. 이제 남은 것이 없을 때까지 돕니다.
- 새 가드는 250행을 넣고 `150 of 250 rows are still waiting after one call to ProcessPending`으로 실패합니다. 그 가드는 **자기 사이트를 스스로 만듭니다** — 첫 판은 기존 사이트를 빌렸다가 httpapi 픽스처가 실행 중에 지워서 외래 키로 실패했습니다. `go test`는 패키지를 같은 데이터베이스에 대해 동시에 실행합니다.
- 배달 경로가 사이트 시간대를 세 가지 방법으로 읽던 것을 하나로 모았고, 이상 감지 분기는 시간대를 못 읽을 때 **조용히 UTC로 떨어지지 않습니다** — 그건 아무 말 없이 다른 날의 이상을 보고하는 것입니다.

## v0.34.6

- **콘솔에서 만든 Segment를 정기 배달할 수 없었습니다.** Scheduled Report 화면은 "분석에서 Action까지 연결합니다: Segment → Webhook → CRM/Mail/AI Agent/사내 메시지"를 내걸고 `Segment 집계` 종류를 제공하는데, **그 종류가 뜻하던 것은 Event 이름·Feature·부서 세 속성의 일치**였습니다. Segment 화면이 만드는 중첩 조건, 사람의 전체 이력에 대한 행동 집계(`entity.sessions >= 3 AND entity.conversions = 0`), 막힘 신호 규칙은 **하나도 배달할 수 없었습니다.** 같은 낱말이 어느 문으로 들어왔느냐에 따라 다른 사람들을 가리켰고, 작동하지 않던 쪽이 제품이 내세우는 그 선이었습니다.
- **실수가 아니라 구조였습니다** — 컴파일러가 HTTP 패키지 안에 있었고 스케줄러는 그것을 import할 수 없습니다(`httpapi`→`service` 한 방향). `internal/segment`로 옮겼습니다: 콘솔도, MCP도, 정기 배달도 **같은 정의를 평가해야 하는 것**이기 때문입니다. `httpapi`는 예전 이름을 별칭으로 유지해 호출부는 그대로입니다.
- 정의가 **저장된 Segment를 가리킬 수 있고**, 화면이 쓰는 컴파일러가 그대로 평가합니다. 속성 세 조건은 남아서 그 위에 **겹쳐 좁힙니다** — 기존 스케줄이 쓰고 있고, Segment를 저장하지 않고 좁은 질문을 던지는 정당한 방법입니다. 스케줄러는 주체 없이 읽고(새벽 2시에는 아무도 없습니다), 다른 사이트에 닿지 못하게 막는 것은 사이트 범위입니다.
- 가드는 실제로 배달해 **같은 기간·같은 Segment를 읽은 Query Builder**의 인원과 대조하고, **전원을 담은 Segment 위에서는 통과시키지 않습니다** — 사이트 전체를 고르는 정의라면 정의를 무시한 배달도 일치하기 때문입니다.

## v0.34.5

- **정기 배달 다이제스트 두 개가 자기 화면보다 작은 리포트를 보내고 있었습니다.** 화면은 숫자를 차트와 기간 선택기 옆에 놓아 언젠가 누군가 비교하지만, **메일이나 Confluence로 받은 리포트는 혼자 도착합니다** — 옆에 대조할 것이 없습니다.
- **AI 다이제스트에 비용도 성공률도 지연도 모델별 분해도 없었습니다.** 호출 수·사용자 수·입력/출력 토큰만 보냈는데, 이건 v0.34.3 이전 AI 화면의 쿼리가 멈춰 있던 자리이자 MCP 도구가 고쳐지기 전까지 멈춰 있던 자리입니다 — **다이제스트는 같은 쿼리의 세 번째 사본**이었습니다. 주간 AI 다이제스트를 구독하는 사람은 얼마 들었고 잘 돌아갔는지를 묻는데, 둘 다 없었습니다.
- **Experience 다이제스트에 Web Vitals가 한 줄도 없었습니다.** 그 화면의 주제가 Web Vitals인데 오류 수와 영향받은 사용자 수만 보냈고, **오류가 전환에 무엇을 했는지**도 없었습니다 — 오류 수만으로는 그게 문제였는지 알 수 없고, 그게 이 리포트를 보내는 이유입니다.
- 두 쿼리 모두 화면과 MCP 도구가 읽는 `insight`로 옮겼습니다. 합계는 **호출 수로 가중**해 접고(평균의 평균이면 두 번 호출된 모델이 만 번 호출된 모델과 같은 무게를 갖습니다), 고유 사용자 수는 행에서 접어낼 수 없어 자기 읽기를 갖습니다. `insight.Experience`는 절반으로 나뉘어 화면은 **인쇄하지 않는 수치를 사느라 쿼리를 더 내지 않습니다.** `analyze_experience`는 없던 전환 영향을 갖게 됐습니다.
- 가드는 웹훅 경로로 **실제 발송**해 payload를 화면과 대조하고, **사이트 달력으로** 비교합니다 — UTC+9 사이트에서는 두 경계가 서로 다른 UTC 날짜에 떨어져 하루짜리 다이제스트가 이틀짜리 화면과 비교됩니다. 배달된 비용이 0이거나 성공률이 양 끝이거나 오류를 겪은 사람이 없으면 **실행을 거부합니다.**

## v0.34.4

- **에이전트에게 목표 달성 여부를 물으면 목표를 되읽어 주고 있었습니다.** `get_metric_goals`는 "목표와 **현재 달성 상태**를 조회합니다"로 소개되는데, 실제로는 `metric_goals` 정의(이름·지표·목표값·비교연산자·기간·담당자)만 답하고 **달성 상태가 한 글자도 없었습니다.** 숫자가 어긋난 게 아니라 애초에 없어서, 읽는 쪽이 틀렸다는 것을 알아차릴 수 없는 종류의 답입니다. 목표 화면의 평가를 하나의 메서드로 꺼내 양쪽이 부르게 했고, 이제 도구도 현재값·달성 여부·진척률·전망을 함께 답합니다.
- **Retention이 아직 판단할 수 없는 Cohort를 "안 돌아온 Cohort"로 세고 있었습니다.** `analyze_retention`은 Cohort 쿼리의 사본을 갖고 있었고, 사본은 원본과 일치했지만 **성숙도가 빠져 있었습니다.** 기간이 닫히기 나흘 전에 시작한 Cohort는 아직 1주차가 없는데, 화면의 통합 곡선은 `cohortMaturity`로 그것을 제외하고 도구의 행은 **아무도 돌아오지 않은 Cohort**로 내놓았습니다. 에이전트가 평균을 내면 어린 Cohort들만큼 정확히 Retention이 낮게 나옵니다 — 낮은 숫자는 그럴듯해 보여 아무도 반박하지 않습니다. 이제 화면과 같은 `runCohortGrid`·`pooledRetentionCurve`를 실행합니다.
- 두 비교 모두 **0끼리 일치하는 것을 통과로 세지 않습니다** — 모든 목표가 0으로 평가되거나 0주차 이후 돌아온 사람이 없으면 실행을 거부합니다. 결함을 되돌리면 각각 빠진 필드 이름과 `pooled week 11 cohort_users: the tool says 2 and the screen says 0`으로 실패합니다.

## v0.34.3

- **도커 이미지 이름을 `momento:v<version>`으로, 릴리스 아카이브를 `momento-v<version>.tar.gz`로 정리했습니다.** 릴리스는 `-t momento-v0.34.2`로 빌드하고 있었는데, 도커는 이것을 **태그 없는 저장소 이름**으로 읽어 `momento-v0.34.2:latest`로 저장합니다 — 버전이 저장소 이름에 들어가고 태그는 아무 말도 하지 않습니다. Makefile의 로컬 타깃은 `momento:0.34.2`로, 버전은 제자리지만 다른 모든 곳이 달고 있는 `v`가 빠져 있었습니다. **아카이브 이름은 그대로**이므로 게시된 자산과 기존 링크는 바뀌지 않고, 그 안의 이미지 이름만 달라집니다.
- 두 이름은 **의도적으로 나란하고, 종류가 달라서 형태가 다릅니다** — 이미지는 저장소와 태그, 아카이브는 한 단어. 오프라인 가이드가 그 둘을 나란히 인쇄하는 곳이라 중요합니다: 지금은 `docker load`가 `Loaded image: momento:v0.34.3`을 출력하고 그게 바로 다음 명령에 쓰는 이름입니다. 빌드·저장·체크섬·로드까지 실제로 해서 확인했습니다.
- **MCP 도구가 자기 화면보다 작은 리포트를 답하고 있었습니다.** `analyze_ai_operations`의 쿼리는 토큰 수에서 멈춰 있어서, 화면에는 있는 **성공률·비용·폴백 수**가 도구에는 없었습니다. 에이전트에게 AI 사용 비용을 물으면 **비용이 없는 리포트**를 받습니다 — 틀린 숫자는 아니지만 읽는 사람에게 의심할 거리조차 없어 더 나쁩니다. 이제 하나의 정의를 양쪽이 부릅니다.
- `get_workspace_rollup`과 `get_feature_scores`도 화면과 행·필드 단위로 대조했습니다. 둘은 이미 일치했고, 하나가 어긋난 뒤가 아니라 비교를 쓰는 김에 고정했습니다.

## v0.34.2

- **MCP 도구가 화면과 다른 답을 하고 있었습니다.** `analyze_internal_usage`는 사내 사용 현황 화면과 같은 질문에 — 읽는 사람 대신 에이전트에게 — 답하는데, 둘이 각자 사본을 갖고 있었고 **그룹핑이 어긋날 수 있는 세 가지 모두에서** 어긋나 있었습니다: 빈 속성을 화면은 `(미지정)`으로 접지만 도구는 독자 라벨로 뒀고, 화면은 값이 없으면 사이트 자신의 서비스 이름으로 대체하지만 도구는 `(미지정)`이라 했고, 화면은 버튼 이름에 `element_id`까지 보지만 도구는 보지 않았습니다. 픽스처에서 이건 **화면이 `포털`이라 부르는 것을 도구가 `(미지정)`이라 부르는** 차이입니다.
- **MCP 도구들은 "에러 없이 답하는지"만 검사되고 있었습니다.** 22개 전부 호출되지만 **무엇이라고 답하는지는 아무도 확인하지 않았습니다** — 그래서 이 결함이 통과하는 테스트 안에 앉아 있었습니다. 틀린 숫자가 살기에 더 어려운 자리이기도 합니다: 화면은 차트와 기간 선택기 옆에 숫자를 놓아 결국 누군가 비교하지만, **에이전트는 문장 하나로 숫자를 말하고 옆에 아무것도 없습니다.**
- 자기 SQL을 가진 도구 셋(검색·데이터 품질·이커머스)을 화면과 값 단위로 비교합니다. **비교는 0이 아닌 숫자로 해야 합니다** — 픽스처에는 검색이 없고 수집기를 통과한 것도 없어서 양쪽이 0을 답하며 **무엇이 틀렸든 일치**했습니다. 그래서 검사할 활동을 먼저 수집기로 전송하고, 각 비교는 화면이 판별 지표(검색 수·탐지된 PII·매출)에서 0을 보고하면 **실행을 거부합니다.**
