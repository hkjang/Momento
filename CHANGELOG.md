...
- **같은 서열 규칙이 화면에도 한 곳에 있습니다.** `web/src/pages/roleScope.ts`의 `ROLE_ORDER`·`roleRank`·`canAdministerRole`·`assignableRoles`는 `internal/auth/auth.go`의 `roleRank`와 같은 순서이고, 모르는 **대상** 역할은 서버의 맵 기본값처럼 0이 됩니다. **호출자** 역할을 모를 때만 서버보다 엄격하게 닫습니다 — 화면이 편집을 내주지 않는 쪽이 무해한 방향이기 때문입니다. 서버 검사가 여전히 경계이며, 화면이 가린다는 이유로 느슨해진 곳은 없습니다.
- 편집할 수 없는 행의 버튼은 **비활성**이고 Tooltip이 이유를 말합니다 — 「내 권한보다 높은 계정은 편집할 수 없습니다」(비활성 버튼은 포인터 이벤트를 내지 않으므로 `span`으로 감쌌습니다). 두 권한 select는 `assignableRoles(내 역할)`만 나열하므로 부여할 수 없는 역할이 목록에 없습니다.
- `web/test/roleScope.test.mjs`가 서버 `roleRank` 복제본과 5×5 순회로 두 규칙이 갈라지지 않음을 고정합니다. Go·SDK·API·데이터베이스 변경은 없습니다.
...
- **편집 대상 계정도 부여하는 역할과 같은 방식으로 제한됩니다.** `super_admin`의 비밀번호를 설정할 수 있는 `organization_admin`은 `super_admin`으로 로그인할 수 있고, 강등하거나 비활성화할 수 있다면 배포에 관리자가 하나도 남지 않을 수 있습니다. 자기보다 높은 역할의 계정에는 `ROLE_ABOVE_CALLER`(403)로 답합니다. 자기 자신은 `SELF_PASSWORD`(400)로 거부하고 현재 비밀번호를 묻는 프로필로 보냅니다. 없는 id는 조용한 200이 아니라 404입니다.
- **12자 최소 길이가 글자 수를 셉니다.** `PasswordProblem`이 바이트를 세어 한글 4자(12바이트)가 콘솔이 "12자 이상"이라고 적은 검사를 통과했습니다. `utf8.RuneCountInString`으로 세어 문구와 검사가 같은 것을 뜻합니다. 72바이트 상한은 bcrypt의 한계이므로 그대로 바이트입니다.
- 콘솔의 사용자 편집에 **「비밀번호 재설정」** 항목이 로컬 계정(OIDC 아님)이면서 자기 자신이 아닐 때 나타나고, 비워 두면 그대로입니다. 두 한계를 모두 말하는 문구와 검사는 `web/src/pages/passwordRule.ts` 한 곳에서 나오며 사용자 생성·프로필·재설정이 같은 것을 씁니다.
...
- **긴 비밀번호로 프로필을 한 번 저장하면 계정이 영구히 잠겼습니다.** bcrypt는 72바이트를 넘는 입력을 거부하는데 `updateMe`는 그 오류를 보지 않고 **빈 결과를 그 사람의 해시 위에 덮어썼고**(한글 25자면 충분합니다), 데이터베이스 바깥에서 비밀번호를 되돌리는 경로가 없습니다. `createUser`도 같아서 아무도 들어갈 수 없는 계정이 주소를 차지했습니다. 이제 해시 전에 길이를 확인하고, 해시나 UPDATE 실패는 `updated:true` 대신 실패로 보고하며, 감사 로그가 비밀번호가 바뀌었는지를 적습니다.
- **방문자가 쓴 셀은 스프레드시트에서 텍스트입니다.** Raw CSV Export와 콘솔 표 CSV의 거의 모든 열은 방문자의 브라우저가 쓴 것이고, Excel·LibreOffice는 `=` `+` `-` `@`로 시작하는 셀을 수식으로 읽습니다. `exportEvents`와 `DataTable`이 그런 셀 앞에 아포스트로피를 붙입니다. id·시각·JSON·숫자·보통 문자열은 그대로입니다.
- **Segment의 contains·startsWith·endsWith와 방문자 검색이 입력값을 ILIKE 패턴으로 읽었습니다.** `report_daily`는 `reportXdaily`도 잡았고, URL 인코딩된 한글 경로 `%EA%B0%80`은 그 16진수가 순서대로 있는 모든 페이지와 일치했습니다 — 더 느슨한 모집단이 아니라 틀린 모집단. `segment.LikeLiteral`이 세 문자를 이스케이프하고 두 경로가 그것을 거칩니다.
- **「추적」 버튼이 기본 정책의 모든 사이트에서 `RANGE_EXCEEDS_POLICY`로 실패했습니다.** 방문자 추적은 1년치를 요청했고 기본 정책은 대화형 읽기 180일입니다. 기간을 고정한 열 개의 fetch가 `policyRange`를 거치고, 소스 스캔 테스트가 그렇지 않은 새 `rangeQuery(<숫자>, …)`를 거부합니다.
- **Event Catalog**가 이벤트를 쓰는 메트릭을 정의 텍스트에 대한 LIKE로 세어 `purchase`가 `purchase_completed`의 메트릭과 일치했습니다 — 정의 트리의 `event_name` 값을 jsonpath로 비교합니다. **Data Lineage**는 최상위만 읽어 ratio가 아무도 먹이지 않는 메트릭으로 보였습니다 — 분자·분모를 따라가고 `metric_goals` 질의는 오류를 버리지 않습니다.
- **이벤트 워커와 교착에서 진 재구축은 실패가 아니라 다시 시도됩니다** — 70일 백필이 재구축 17건을 그 이유로 failed로 남겼습니다. Aggregate 탭은 사유가 적힌 pending 행을 **재시도 대기**로 표시하고 시도 횟수를 보여 줍니다.
- 사이트를 볼 수 있는가의 판정이 아홉 곳에 손으로 적혀 있었습니다 — `siteVisibleTo`·`siteAdministeredBy`로 모았습니다. 어긋난 사본은 실패하지 않고 남의 데이터를 담아 대답합니다.
- **마이그레이션은 advisory lock 아래 한 번에 하나만 돕니다.** 같은 데이터베이스를 여러 프로세스가 동시에 마이그레이션하면 늦은 쪽이 `CREATE TYPE`·`CREATE EXTENSION`의 카탈로그 유일 인덱스에 걸렸습니다.
- `compose.yml`이 적재한 릴리즈 이미지는 `--no-build`로 띄운다고 적습니다. 사용자·관리자 가이드를 실제 화면 캡처가 실린 완성본으로 다시 썼고 `scripts/guide/`로 다시 찍을 수 있습니다. CI: httpapi fixture를 한 배치로(5분 35초→56초), 사용량 비율 검사는 읽기가 겹칠 수 있는 곳에서만, js-yaml 4.3.2.

## v0.34.39

- **25초 분석 제한 규칙이 `analytics_events`라는 View 이름 하나로 쓰여 있어서, 그 View를 만드는 테이블로 같은 데이터를 읽는 여섯 핸들러를 본 적이 없었습니다.**
- **Tracking Debugger**가 가장 나빴습니다 — **지금까지 도착한 모든 이벤트를 관리자가 볼 수 있는 모든 사이트에 걸쳐 정렬해** 200건을 가져옵니다. 기간 제한도, 그 정렬을 받치는 인덱스도 없습니다. 그리고 이 화면은 **수집이 멈춘 것 같을 때 여는 화면**이므로, 테이블이 가장 크고 답이 가장 중요할 때 실행될 가능성이 가장 높습니다.
- **Event Catalog**는 정의마다 `raw_events` 전체를 기간 제한 없이 세고, **Identity 리포트**는 사이트의 `visitor_identities` 전체를 GROUP BY 하며, **Session·Path 리포트와 설치 진단**은 읽는 사람이 고른 기간을 그대로 읽습니다. 어느 것도 안내와 함께 실패할 수 없었고 브라우저가 포기할 때까지 커넥션을 붙들었습니다.
- `observedOrigins`가 Request 대신 Context를 받아 진단 화면의 읽기도 같은 제한 안에 들어갑니다.
- **규칙은 이제 읽기를 이름으로 부릅니다** — `analytics_events`와 그 아래 event·session·visitor 테이블. 세 핸들러는 면제이고 **테스트가 이유를 적습니다**: 두 Export는 의도적으로 스트리밍하고, `deleteAnalyticsData`는 의도적으로 파괴합니다. **아무도 적어 두지 않은 면제는 구멍과 구별되지 않습니다.**

## v0.34.38

- **v0.34.30의 규칙이 `Scan`이라는 이름으로 쓰여 있어서, `rows.Values()`로 읽는 세 반복문이 그대로 지나갔습니다.**
- **쿼리 빌더**는 실패한 행을 `continue`로 버렸습니다 — 사람이 행동할 작정으로 질문을 던지는 화면에서, 답이 **데이터가 가진 것보다 작은 숫자**가 되고 응답 어디에도 **정말 그만큼 조용했던 기간과 다른 점이 없었습니다.**
- **두 Event Export**는 오류를 버리고 곧바로 슬라이스를 인덱싱했습니다 — `Values`가 오류와 함께 돌려주는 **nil**에 `vals[13]`. panic은 recover되고 `writeError`가 **이미 200이고 이미 다운로드인 응답**에 쓰이므로, **잘린 NDJSON에 JSON 오류 객체가 중간에 덧붙은 파일**이 도착하고 **끝나지 않았다고 말하는 상태 코드는 없었습니다.** 두 Export 모두 이 경우를 위한 안내 문구를 갖고 있었지만 거기 도달한 적이 없었습니다.
- **진짜 고침은 규칙입니다.** 반복문이 우연히 부르는 메서드 이름으로 쓴 규칙은 누군가 떠올린 호출만 덮습니다. 이제 **읽기**를 이름으로 부르고(`Scan`·`Values`·`RawValues`), 오류를 버리는 세 번째 방법인 **`_`에 대입하기**를 추가했습니다.

## v0.34.37

- **이상 감지가 하지 않은 비교로 알림을 설명하고 있었습니다.** 이 감지기는 사내 서비스의 요일 주기 때문에 **같은 요일끼리** 비교하고, 같은 요일 표본이 3개 미만이면 최근 28일 평균으로 대체합니다. 그런데 판정 근거 문장은 **어느 쪽이든 `같은 요일 기준선`**이라고 적었습니다. **배포 3주가 지나지 않은 사이트는 항상 대체 기준선을 쓰므로**, 그 기간의 모든 알림이 **주말이 섞인 창과 견준 월요일**을 **월요일들과 견준 월요일**이라고 설명했습니다. 두 경우의 숫자는 똑같이 생겨서, 무엇과 비교했는지 말하는 것이 유일한 일인 필드 말고는 읽는 사람에게 알려 줄 것이 없었습니다.
- `Anomaly`가 `baseline_kind`를 갖고 근거 문장이 그것을 이름으로 부릅니다 — `같은 요일 8주 기준선` 또는 `최근 10일 기준선(같은 요일 표본 부족, 요일 혼합)`. 알림 상태 전이가 근거를 그대로 옮기므로 Webhook·메일이 화면과 같은 말을 합니다.
- **감지 자체는 바뀌지 않았습니다.** 대체 기준선 판정의 강도를 낮추는 것은 별개의 결정입니다.

## v0.34.36

- **같은 설정 화면의 두 목록이 서로 다르게 매칭했습니다.** `token` 규칙은 `?token=`을 마스킹하고 `?Token=`은 통과시켰지만, 같은 규칙이 `Token`이라는 Property는 차단했습니다 — **화면에는 어느 쪽이 어느 쪽인지 없었습니다.** 엔터프라이즈 애플리케이션이 보내는 파라미터는 소문자만큼 자주 PascalCase(`?SessionId=`, `?Email=`)이므로, 자기 URL에서 본 이름을 입력한 운영자는 **동작하는 규칙이나 아무것도 하지 않는 규칙** 중 하나를 얻었고 그 차이는 저장된 행을 열기 전까지 보이지 않았습니다.
- **세는 쪽과 지우는 쪽이 달랐습니다.** `countPrivacyBlocked`는 설정된 이름을 trim했고 `filteredProperties`는 하지 않아, `" email "`처럼 공백이 붙어 저장된 규칙(콘솔은 trim하지만 API는 그대로 받습니다)이 **Data Quality 화면에 "차단됨"으로 보고되고 값은 저장됐습니다.** 규칙이 동작한다고 말하는 것이 유일한 일인 숫자가, 행 안에 그대로 있는 값에 대해 그렇게 말했습니다. 이제 하나의 `matchSet`이 양쪽이 비교하는 것을 만듭니다.
- **화면이 이 설정들이 무엇에 적용되는지 말합니다.** 브라우저 SDK는 Query String을 전송하지 않고 `utm_*`와 설정된 검색 파라미터만 읽어 origin + pathname을 보냅니다. 따라서 마스킹 목록과 「URL Query String 전체 제거」는 **Server API Key로 전송되는 Event**에 적용됩니다.

## v0.34.35

- **보존 작업이 `visitor_sessions`에서 행을 지운 적이 한 번도 없었습니다.** 이 테이블은 site·visitor·session과 **수집기가 뒤에 채우는 SSO User ID**, 그리고 그 사람이 그 세션에서 처음·마지막으로 보인 시각을 갖고 있습니다 — 신원 테이블과 같은 매핑입니다. v0.32.4의 수정이 `visitor_identities`·`visitors`·`identified_users`를 덮으면서 이 테이블을 지나쳤고, 그래서 **Raw Event와 Session이 사라진 뒤에도 사람 단위 기록이 무기한 남았습니다.** 의무를 지키려고 보존 기간을 설정한 운영자는 그것을 지키지 못한 상태였습니다.
- **개인 단위 삭제는 영향이 없었습니다** — 개인정보 요청 경로는 살아남은 이벤트로부터 파생 테이블을 재구축하기 때문입니다. 지나친 것은 증분으로 지우고 아무것도 재구축하지 않는 **기간 기반 보존**입니다.
- 이 테이블은 세션을 기술하므로 **세션과 함께 만료**합니다 — `sessions`가 쓰는 것과 같은 규칙, 같은 정책 컬럼. 테스트는 400일 전 행과 한 시간 전 행을 한 달 보존 기간 아래 두고 앞만 사라질 것을 요구합니다.
- 관리자 가이드의 보존 적용 범위 표에 이 색인을 명시했습니다.
...
- 네트워크 사실에는 이미 자기 자리가 있었습니다 — `is_internal` 컬럼과 Segment 필드 `traffic.internal`. **이제 각자 하나씩만 말합니다.** 사내망 크롤러는 `known_bot`이면서 `traffic.internal = true`입니다.
- 이전에 수집된 Event는 저장된 값을 유지하고, `internal_traffic`은 Semantic Metric에서 계속 유효하므로 저장한 정의는 그대로 열립니다.
- 테스트는 테스트 클라이언트의 대역을 사내망으로 등록한 뒤 직원 브라우저와 Uptime 감시를 보내 각각 `normal`·`monitoring`이면서 **둘 다 `is_internal = true`**일 것을 요구합니다.
- 사용자·관리자 가이드의 분류 표와 제외 안내를 고쳤습니다.

## v0.34.32

- **모든 직접 유입이 "사내망"으로 보고되고 있었습니다.** 개요의 유입 채널은 직접 유입을 `Direct`와 `Direct (사내망)`으로 나누는데 — 온프레미스 제품에서 그 행이 존재하는 이유인 구분입니다 — **"이벤트에 네트워크 이름이 붙어 있는가"**로 판단했습니다. **수집기는 모든 이벤트에 이름을 붙입니다**(대역에 맞지 않으면 `External / Unclassified`). 그래서 조건은 트래커로 도착한 **모든** 이벤트에 참이었고, **네트워크 대역을 하나도 설정하지 않은 사이트** — 정의상 모든 방문이 외부인 사이트 — 에서도 전부 사내망으로 보고됐습니다. 기록하는 컬럼은 같은 행의 `is_internal`이었습니다.
- 테스트는 대역 밖 주소에서 수집기를 통해 직접 방문을 넣고 **수집기가 실제로 쓴 값을 확인한 뒤** 사내망 항목에 들어가지 않을 것을 요구하며, 이어 같은 주소를 덮는 대역을 사내망으로 등록해 **안에서 온 방문은 거기 들어갈 것**을 요구합니다.
- Tracking Debugger의 어느 컬럼이 null인지에 대한 주석을 바로잡았습니다 — `network_name`은 외부 트래픽이 아니라 **수집기가 아닌 것이 쓴 행**에서 null입니다.

## v0.34.31

- **"프라이버시 정책이 무엇인가"를 일곱 곳이 각자의 인라인 SQL과 각자의 기본값으로 물었고, 기본값이 서로 달랐습니다.** Visitor Explorer의 다섯 표면 중 **둘은 `visitor_profiles`를 기본값 true로, 셋은 false로** 읽었습니다. 그 항목의 이름을 대지 않는 정책에서는 **타임라인과 검색은 동작하고 프로필 목록·Identity 리포트·`query_identity_graph`는 "관리자가 껐다"고 답했습니다.** 어디에도 오류는 없습니다 — 양쪽 다 쓰인 그대로 동작합니다.
- **Do Not Track은 배포 기본값이 true인데 false로 폴백했고, 설정 읽기가 실패하면 추적을 거부한 사람의 이벤트를 수집했습니다.** 모르는 것이 곧 수집하지 않는 것이어야 하는 자리입니다.
- **수집기는 정책을 결과를 버린 채 파싱했습니다.** 파싱되지 않는 값은 구조체 zero value가 되고 그것은 **IP 전체 보존·마스킹 없음·차단 없음·PII 탐지 없음**입니다 — 그렇게 저장되는 동안 콘솔은 설정된 정책을 보여 줍니다. 이제 파싱 실패는 배치를 거절합니다.
- 뿌리는 하나입니다: 이 보호 설정들에서 **zero value는 제품이 배포하는 값의 정반대**입니다. `internal/privacy`가 정책·배포 기본값·기본값 위에 얹는 파싱을 한 곳에서 갖습니다.
- **`PUT /api/v1/settings/{key}`가 그룹 전체를 교체했습니다.** 항목 네 개만 보낸 쓰기가 masked_parameters·blocked_properties·pii_detection_mode를 지웠습니다. 이제 저장된 값 위에 병합합니다.
- **콘솔 프라이버시 폼에는 오류 분기가 없었습니다.** 설정 읽기가 실패하면 **모든 보호 장치가 꺼진 화면**이 그려지고 클릭 한 번이면 그대로 기록됐습니다.
- 세 가지 규칙을 추가했습니다 — 기본값을 마이그레이션 seed에 붙들고, 다른 곳에서 프라이버시 항목을 꺼내는 것을 금지하고, 항목이 없는 정책에서 다섯 표면이 같은 답을 낼 것을 요구합니다.

## v0.34.30

- **중간에 멈춘 읽기가 짧은 목록이 되어 200과 함께 돌아가고 있었습니다.** 이 서비스의 행 반복문 93개 중 **52개가 행이 오지 않은 이유를 버렸고**, 그 밖의 **28개는 `rows.Err()`는 보면서 스캔 실패는 전부 떨어뜨렸습니다.** 스캔 실패는 나쁜 행 하나가 아닙니다 — 스캔은 모든 행에 같으므로 null을 돌려주기 시작한 컬럼 하나가 **모든 행을 한꺼번에 실패시키고** 화면은 빈 상태를 그립니다. 이 제품에서 빈 상태는 "여기서는 아무 일도 없었다"는 뜻이고, 그것이 아무도 보지 못했을 때 절대 말해서는 안 되는 단 하나입니다.
- **Data Quality 리포트**는 두 쿼리 오류를 모두 버리고 rows 핸들의 nil 여부만 봤기 때문에, 읽기가 실패하면 **"카디널리티 높은 차원 없음"과 "이슈 없음"**을 그렸습니다.
- **암호화 상태**는 세 컬럼을 세다가 실패하면 그때까지의 숫자를 그대로 답해, 끝까지 읽지 못한 컬럼에 대해 **복구 불가 키 0개**라고 보고할 수 있었습니다.
- **재암호화는 읽지도 못한 컬럼에 대해 "N건 재암호화, 0건 실패"라고 답했습니다.** 다음 키 폐기 때 여전히 옛 키로 봉인된 비밀값이 사라집니다. 이제 처리한 것을 감사 로그에 남기고 `REKEY_INCOMPLETE`로 컬럼을 지목하며 거절합니다.
- Visitor Search의 네 반복문은 실패로 빠져나갈 때 rows를 닫고 나갑니다.
- `TestEveryRowLoopReportsItsFailures`가 `internal/`의 모든 반복문을 읽고, 스캔 오류를 묶어 반복문 밖으로 내보낼 것(맨 `continue`는 같은 침묵입니다)과 뒤이어 `rows.Err()`를 확인할 것을 요구합니다 — 읽기는 행과 행 사이에서도 멈추고, 그건 `Scan`이 보지 못합니다.

## v0.34.29

- **Tracking Debugger가 네트워크 없는 이벤트를 전부 버리고 있었습니다.** `network_name`을 `string`으로 읽는데 그 값은 **이름 붙은 사내망 밖에서 온 모든 것에 대해 null**입니다 — 대부분의 사이트에서 대부분의 이벤트이고, **사내망을 설정하지 않은 사이트에서는 모든 이벤트**입니다. 스캔은 그 행들 전부에서 실패했고 실패는 버려졌으므로, **운영자가 이벤트 도착을 지켜보려고 여는 화면이 그것들을 조용히 빠뜨렸고**, 사내망이 없는 사이트에서는 아무것도 보여 주지 않았습니다 — **"아무것도 도착하지 않고 있다"와 정확히 같은 모습**으로. 이 화면이 존재하는 이유인 바로 그 결론입니다.
- **원인은 열일곱 곳이 지나가는 헬퍼였습니다.** `rowsToList`는 스캔 실패한 행을 **버리고** `rows.Err()`도 보지 않았습니다. 그리고 **스캔 실패는 나쁜 행 하나가 아닙니다** — 스캔은 모든 행에 같으므로 타입이 안 맞게 된 컬럼 하나가 **모든 행을 실패시키고**, 화면은 **데이터 없는 사이트와 똑같이 생긴 빈 목록**을 그립니다. (이 서비스의 행 반복문 93개 중 **52개**가 `rows.Err()`를 보지 않습니다.)
- 다른 열여섯 곳은 이제 주변 코드가 하는 방식대로 실패를 보고합니다 — 클로저는 오류를 반환하고 핸들러는 `writeQueryError`로 답합니다. 테스트는 네트워크 없는 이벤트 수를 세고 디버거가 그중 일부를 보여 줄 것을 요구합니다.

## v0.34.28

- **개인정보 내보내기가 한 행도 읽기 전에 "완전하다"고 선언하고 있었습니다.** 사람이 *"나에 대해 무엇을 갖고 있느냐"* 고 물었을 때 받는 파일인데, 응답 헤더와 감사 기록에 **완전함을 먼저 쓰고** 스트리밍했습니다 — **주장이 먼저 나가고 되돌릴 수 없습니다.** 중간에 실패한 읽기는 여전히 완전하다고 말하는 **더 짧은 파일**을 만들었고, `rows.Err()`는 확인되지 않았고, **디코딩되지 않는 행은 `continue`로 건너뛰었습니다** — 물어본 사람에게서 데이터를 빼놓는 일을, 주석을 달자면 "넘어감"이라고밖에 쓸 수 없는 한 줄이.
- **이제 보내기 전에 만듭니다.** 모든 실패는 파일 대신 **오류**로 답하고, 헤더와 감사 기록은 **참이 된 뒤 마지막에** 씁니다. 너무 큰 내보내기는 상태 코드와 해결책과 함께 **거부**합니다 — 요청에는 선택적 기간이 있어 좁히는 것은 운영자가 할 수 있는 일이고, **완전하다고 주장하는 반쪽 파일은 아무도 할 수 있는 일이 없습니다.**
- **같은 읽기가 삭제 경로에도 있었고 거기서도 오류를 무시했습니다.** 거기서 놓친 기기는 더 짧은 파일이 아니라 **그 사람이 지워 달라고 한 데이터가, 완료로 기록된 요청 안에서, 그대로 남아 있는 것**입니다.
- 기존 테스트는 200에 빈 본문이 아닌지만 봤습니다. 이제 **데이터베이스가 그 사람에 대해 갖고 있는 건수**를 세어 파일이 정확히 그만큼 담는지 요구합니다 — 한 건만 빼면 `the export carries 1 events and the database holds 955 for this person`으로 실패합니다.

## v0.34.27

- **짧게 끝난 내보내기가 온전해 보였습니다.** 내보내기는 스트리밍이고 첫 행과 함께 헤더가 나가므로 **무언가 잘못될 시점에는 이미 200이고 이미 다운로드**입니다 — 바꿀 상태가 남아 있지 않습니다. 그래서 파일은 짧게 끝나고 **깨끗하게 열리는 CSV, 아무도 짚을 수 없는 빠진 행**, 끝까지 읽히지 않은 기간을 대신하는 파일이 되었습니다.
- 두 경로 다 흔적이 없었습니다. **`rows.Err()`를 한 번도 읽지 않아** 중간에 실패한 읽기가 그냥 루프를 끝내고 반환했고(csv writer 오류도 버려졌습니다), **행 수 제한이 조용했습니다** — 10만 건은 실패가 아니지만 **정확히 10만 건을 받은 사람은 완전한 것과 잘린 것을 구분할 수 없고**, 그 차이는 그들이 갖고 있다고 생각하는 기간입니다.
- 이제 **마지막 줄이 어느 쪽인지 말합니다**: 실패는 이유와 몇 건까지 나갔는지, 제한은 도달한 숫자와 어떻게 해야 하는지. NDJSON은 객체 하나, CSV는 **`#momento`로 시작하는 보이는 행**. **완전한 내보내기는 아무것도 덧붙이지 않습니다** — 온전한 파일에 붙는 안내는 안내를 무시하는 법을 가르칩니다. 제한은 이름 붙은 상수라 쿼리와 메시지가 어긋날 수 없습니다.
- 보다가 확인하고 멀쩡했던 것: CI가 **콘솔·SDK 스위트도** 돌리고 콘솔 빌드가 `tsc -b`라 타입 오류가 main에 닿을 수 없으며, 정기 배달 **여덟 종류 전부**가 payload를 들여다보는 테스트로 검사됩니다.

## v0.34.26

- **Funnel의 경로 분석이 사이트가 거부할 기간을 제시하고 있었습니다.** 기간 규칙은 *두 공유 컨트롤이 정책을 적용하는가*와 *그것을 붙이는 화면이 한도를 넘기는가*를 보는데, **둘 다 "공유 컨트롤을 쓰는 화면"에 대한 것**입니다. 경로 분석은 둘 중 어느 것도 쓰지 않고 **7·30·90을 하드코딩**했습니다 — `AnalysisToolbar`가 갖고 있던 바로 그 결함이, **그것을 잡은 규칙이 들여다보지 않는 유일한 화면**에서. 규칙이 컨트롤을 기준으로 쓰였기 때문에 **컨트롤을 쓰지 않는 화면이 지나갔습니다.**
- 14일로 제한된 사이트에서 그 선택기는 `RANGE_EXCEEDS_POLICY`로 답할 기간 두 개를 제시했습니다. 이제 다른 모든 곳처럼 `RangeSelect`를 지나갑니다.
- **규칙은 질문을 반대로 놓습니다**: *어떤 화면도 자기 기간 선택지를 직접 그려서는 안 된다.* 선택기를 되돌리면 파일 이름과 제시한 숫자를 대며 실패합니다 — 전에는 아무것도 보고하지 않았습니다.

## v0.34.25

- **규칙에 정확히 MCP 크기만큼 구멍이 뚫려 있었습니다.** *"모든 무거운 조회는 분석 제한 시간 아래에서"* 규칙의 테스트가 `(w, r)` 모양 핸들러만 찾는데, `mcpCall`은 디코딩된 요청을 **세 번째 매개변수**로 받습니다 — 그래서 **도구 스물두 개**(에이전트가 기간과 절단면을 고르고, **일곱은 기간 없이 전체 이력**을 읽는)가 **한 번도 검사되지 않았습니다.** 제품에서 가장 자유롭게 넓혀지는 표면이고, **아무도 스피너를 보며 포기하지 않는** 곳입니다. **구멍이 뚫린 규칙은 규칙과 똑같이 읽힙니다** — "규칙이 닿지 않는 자리 찾기"가 찾았고 코드를 읽는 것은 못 찾았습니다.
- 매처는 이제 **매개변수가 몇 개든** 응답 writer와 요청을 받는 함수를 찾아 **열일곱 개**를 봅니다. 제한 시간은 변수가 아니라 **요청에 붙입니다** — 도구들이 `*http.Request`를 받아 스스로 컨텍스트를 읽는 헬퍼로 데이터베이스에 닿기 때문입니다.
- **제한 시간이 생기면 할 말도 필요합니다.** 도구는 드라이버의 말을 그대로 답해서, 시간이 다 된 조회가 에이전트에게 `context deadline exceeded`라고 말했습니다 — **행동할 것이 없고, 그런 에이전트는 같은 호출을 반복합니다.** 이제 화면과 같은 코드와 안내(기간 줄이기·Segment·정기 배달)를 답합니다. 인식되지 않는 오류는 **자기 말을 유지합니다** — "Semantic metric not found"를 query failure로 꾸미는 건 무엇이 잘못됐는지에 대해 거짓을 말하는 것입니다.
- **이벤트 내보내기는 의도적으로 그대로입니다**: `raw_events`를 읽어 규칙이 부르지 않고 그게 맞습니다 — 내보내기는 다운로드이지 누군가 기다리는 화면이 아니며, 이미 행 수 제한과 클라이언트가 떠나면 취소되는 요청으로 묶여 있습니다.
