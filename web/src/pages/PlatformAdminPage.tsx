...
function Governance() {
  ...
  const contractSave = useMutation({
    ...
    onSuccess: () => {
      setEventName("");
      void qc.invalidateQueries({ queryKey: ["event-contracts"] });
    },
  });
  const metricSave = useMutation({
    mutationFn: () =>
      post(`/api/v1/sites/${site!.site_id}/semantic-metrics`, {
        name: metricName,
        label: metricName,
        description: "Admin Console metric",
        definition: JSON.parse(metricDefinition),
        format: "number",
        status: "active",
      }),
    onSuccess: () => {
  ...
  const targetSave = useMutation({
    ...
    onSuccess: () => {
      ...
      void qc.invalidateQueries({ queryKey: ["adoption-targets"] });
    },
  });
  ...
  const activate = useMutation({
    mutationFn: (row: { event: string; version: number }) =>
      post(
        `/api/v1/sites/${site!.site_id}/event-contracts/${encodeURIComponent(row.event)}/${row.version}/activate`,
      ),
  ...
  const ciCheck = useMutation({
    mutationFn: () =>
      post<CIValidation>(
        `/api/v1/sites/${site!.site_id}/event-contracts/validate`,
        { environment, ...(JSON.parse(ciPayload) as Record<string, unknown>) },
      ),
  });
  ...
  if (envs.isLoading || contracts.isLoading || metrics.isLoading)
    return <Loading />;
    ...
    <Stack spacing={3}>
      <Card sx={{ p: 2.5 }}>
        ...
        <Typography variant="body2" color="text.secondary" mb={2}>
          SDK/Server API의 환경을 물리적으로 분리하고 계약 강도와 일별
          Cardinality 상한을 관리합니다.
        </Typography>
        <Stack spacing={1.5}>
          {envs.data?.map((item) => (
            <EnvironmentEditor
      ...
      <Card sx={{ p: 2.5 }}>
        ...
        <Typography variant="body2" color="text.secondary" mb={2}>
          Draft를 만들고 활성화한 버전만 수집 계약의 기준이 됩니다. 현재 선택
          환경은 {environment.toUpperCase()}입니다.
        </Typography>
        <Box
          sx={{
            display: "grid",
        ...
        >
          <Stack spacing={1.5}>
            ...
            <Button
              variant="contained"
              disabled={!eventName || contractSave.isPending}
              onClick={() => contractSave.mutate()}
            >
              Version 생성 및 활성화
            </Button>
            {contractSave.error && <ErrorState error={contractSave.error} />}
          </Stack>
          <TextField
            multiline
            minRows={7}
            label="JSON Schema"
            value={eventSchema}
            onChange={(e) => setEventSchema(e.target.value)}
          />
        </Box>
        <Divider sx={{ my: 2 }} />
        <DataTable
          ...
          columns={[
            ...
            { key: "validation_mode", label: "Validation" },
            { key: "changelog", label: "Changelog" },
            {
              ...
              format: (v, row) =>
                row.status === "active" ? (
                  <Chip size="small" color="success" label="사용 중" />
                ) : (
                  <Button
                    size="small"
        ...
        {activate.error && (
          <Box mt={2}>
            <ErrorState error={activate.error} />
          </Box>
        )}
        <Divider sx={{ my: 2 }} />
        <Typography fontWeight={700}>배포 전 CI 검증</Typography>
        <Typography variant="body2" color="text.secondary" mb={1.5}>
          배포 파이프라인이 호출하는 것과 같은 검증을 콘솔에서 실행합니다. 활성
          Contract와 환경 정책을 함께 적용합니다.
        </Typography>
        <Stack spacing={1.5}>
          <TextField
            multiline
          ...
          <Box>
            <Button
              ...
              onClick={() => ciCheck.mutate()}
            >
              {environment.toUpperCase()} 기준 검증
            </Button>
          </Box>
          {ciCheck.error && <ErrorState error={ciCheck.error} />}
          {ciCheck.data && (
            <>
              <Alert severity={ciCheck.data.valid ? "success" : "error"}>
                오류 {ciCheck.data.errors}건, 경고 {ciCheck.data.warnings}
                건입니다.
              </Alert>
              <DataTable
                rows={
                  ciCheck.data.results as unknown as Record<string, unknown>[]
                }
                columns={[
      ...
      <Card sx={{ p: 2.5 }}>
        ...
        <Typography variant="body2" color="text.secondary" mb={2}>
          허용된 AST만 SQL로 컴파일해 모든 Dashboard/API/MCP가 같은 지표 정의를
          사용합니다.
        </Typography>
        <Stack direction={{ xs: "column", md: "row" }} spacing={1.5}>
          ...
          <Button
            variant="contained"
            disabled={!metricName || metricSave.isPending}
            onClick={() => metricSave.mutate()}
          >
            저장
          </Button>
        </Stack>
        {metricSave.error && (
          <Box mt={2}>
            <ErrorState error={metricSave.error} />
          </Box>
        )}
        <Box mt={2}>
          <DataTable
            ...
            columns={[
              ...
              { key: "definition_version", label: "Version", align: "right" },
              ...
              { key: "description", label: "Description" },
              {
                ...
                format: (v) => (
                  <Button
                    size="small"
                    onClick={() => setMetricName2(String(v))}
                  >
                    값 조회
                  </Button>
                ),
        ...
        {metricName2 && (
          <Box mt={2}>
            ...
            <Typography fontWeight={700} mb={1}>
              {metricName2} · 최근 30일 · {environment.toUpperCase()}
            </Typography>
            {metricQuery.isLoading ? (
              <Loading />
            ) : metricQuery.error ? (
              <ErrorState error={metricQuery.error} />
            ) : metricQuery.data ? (
              <Alert severity="info">
                {metricQuery.data.label}:{" "}
                {Number(metricQuery.data.value).toLocaleString("ko-KR")}{" "}
                {metricQuery.data.unit} (정의 v
                {metricQuery.data.definition_version}, {metricQuery.data.format}
                )
              </Alert>
            ) : null}
          </Box>
      ...
      <Card sx={{ p: 2.5 }}>
        ...
        <Typography variant="body2" color="text.secondary" mb={2}>
          조직·부서별 실제 대상자 수를 입력하면 Feature Adoption Rate의 분모로
          사용합니다.
        </Typography>
        <Stack direction={{ xs: "column", lg: "row" }} spacing={1.5}>
          <TextField
            ...
            onChange={(e) =>
              setTarget({ ...target, organization: e.target.value })
            }
          />
          <TextField
            label="Department"
            ...
            onChange={(e) =>
              setTarget({ ...target, department: e.target.value })
            }
          />
          <TextField
            label="Feature"
            value={target.feature}
            onChange={(e) => setTarget({ ...target, feature: e.target.value })}
          />
          <TextField
            label="Eligible Users"
            type="number"
            value={target.eligible_users}
            onChange={(e) =>
              setTarget({ ...target, eligible_users: Number(e.target.value) })
            }
          />
          <Button
            variant="contained"
            disabled={!target.feature || targetSave.isPending}
            onClick={() => targetSave.mutate()}
          >
            저장
          </Button>
        </Stack>
              ...
              { key: "eligible_users", label: "대상자", align: "right" },
              {
                ...
                format: (v) => (
                  <Button
                    ...
                    onClick={() =>
                      void del(
                        `/api/v1/sites/${site.site_id}/adoption-targets/${String(v)}`,
                      ).then(() =>
                        qc.invalidateQueries({
                          queryKey: ["adoption-targets"],
                        }),
                      )
                    }
                  >
...
function EnvironmentEditor({
  ...
  const save = useMutation({
    ...
    onSuccess: () =>
      qc.invalidateQueries({ queryKey: ["platform-environments"] }),
  });
  return (
    <Stack
    ...
    >
      <Chip
        ...
        color={item.name === "prd" ? "primary" : "default"}
      />
      <Typography sx={{ minWidth: 130 }}>{item.label}</Typography>
      <TextField
        select
        size="small"
        label="Contract"
        value={mode}
        onChange={(e) =>
          setMode(e.target.value as SiteEnvironment["contract_mode"])
        }
        sx={{ minWidth: 130 }}
      >
        {["allow", "warn", "reject"].map((value) => (
...
function Automation() {
  ...
  // conditions as three properties that mean something narrower.
  ...
  const current = config ||
    settings.data?.automation?.value || {
      enabled: false,
  ...
  useEffect(
    () => setForm((value) => ({ ...value, environment })),
    [environment],
  );
  ...
  if (!site) return <NoSite />;
  if (settings.isLoading || channels.isLoading || schedules.isLoading)
    return <Loading />;
  ...
  const segmentLabel = form.segmentId
    ? String(
        (segments.data || []).find((item) => String(item.id) === form.segmentId)
          ?.name || "",
      )
    : "";
  return (
    <Stack spacing={3}>
      <Alert severity="info">
        모든 전송은 관리자 Allowlist와 Audit Log를 거칩니다. 기본값은
        비활성이고, Entity ID 전달은 기본 0건입니다.
      </Alert>
      <Card sx={{ p: 2.5 }}>
        <Typography variant="h6">Automation Security</Typography>
        <Stack spacing={1.5} mt={2}>
          <TextField
            select
            label="Scheduler"
            value={String(Boolean(current.enabled))}
            onChange={(e) =>
              setConfig({ ...current, enabled: e.target.value === "true" })
            }
          >
          ...
          <TextField
            ...
            onChange={(e) =>
              setConfig({
                ...current,
                allowed_webhook_hosts: e.target.value
                  .split(",")
                  .map((x) => x.trim())
                  .filter(Boolean),
              })
            }
            helperText="예: hooks.internal, *.corp.local"
          />
          <TextField
            type="number"
            label="Delivery Timeout (seconds)"
            value={Number(current.delivery_timeout_seconds || 10)}
            onChange={(e) =>
              setConfig({
                ...current,
                delivery_timeout_seconds: Number(e.target.value),
              })
            }
          />
          <TextField
            type="number"
            label="Segment Entity ID 최대 전달 수"
            value={Number(current.max_entity_ids || 0)}
            onChange={(e) =>
              setConfig({ ...current, max_entity_ids: Number(e.target.value) })
            }
            helperText="0이면 집계만 전달합니다."
          />
          <Button
            variant="contained"
            disabled={!config || configSave.isPending}
            onClick={() => configSave.mutate()}
          >
            보안 설정 저장
          </Button>
        </Stack>
      ...
      <Card sx={{ p: 2.5 }}>
        ...
        <Typography variant="body2" color="text.secondary" mb={2}>
          Webhook, Confluence, Mail Gateway, 사내 메시지, AI Agent endpoint를
          등록합니다. Header 값은 다시 표시되지 않습니다.
        </Typography>
        <Stack spacing={1.5}>
          <TextField
            label="이름"
          ...
          <TextField
            ...
            onChange={(e) =>
              setChannel({ ...channel, channel_type: e.target.value })
            }
          >
            {[
              "webhook",
          ...
          <TextField
            label="Endpoint URL"
            value={channel.endpoint_url}
            onChange={(e) =>
              setChannel({ ...channel, endpoint_url: e.target.value })
            }
          />
          <Box>
            ...
            <Stack spacing={1}>
              {headerRows.map((row, index) => (
                ...
                >
                  <TextField
                    ...
                    value={row.name}
                    placeholder={
                      commonHeaderNames[index % commonHeaderNames.length]
                    }
                    onChange={(e) =>
                      setHeaderRows(
                        headerRows.map((item, position) =>
                          position === index
                  ...
                  <Button
                    ...
                    onClick={() =>
                      setHeaderRows(
                        headerRows.filter((_, position) => position !== index),
                      )
                    }
                  >
                    삭제
              ...
              <Box>
                <Button
                  size="small"
                  onClick={() =>
                    setHeaderRows([...headerRows, { ...emptyHeaderRow }])
                  }
                >
                  Header 추가
                </Button>
              ...
              <Typography variant="caption" color="text.secondary">
                값은 MOMENTO_ENCRYPTION_KEY로 암호화되어 저장되고 목록에는
                이름만 표시됩니다. Host는 재정의할 수 없습니다.
              </Typography>
            </Stack>
          </Box>
          <Button
            ...
            disabled={
              ...
              headerIssues(headerRows).length > 0 ||
              channelSave.isPending
            }
            onClick={() => channelSave.mutate()}
          >
            Channel 등록
          </Button>
          {channelSave.error && <ErrorState error={channelSave.error} />}
        </Stack>
        <Box mt={2}>
          <DataTable
            rows={(channels.data || []) as unknown as Record<string, unknown>[]}
            columns={[
              { key: "name", label: "이름" },
          ...
          {channelDelete.error && (
            <Box mt={1}>
              <ErrorState error={channelDelete.error} />
            </Box>
          )}
        </Box>
      </Card>
      <Card sx={{ p: 2.5 }}>
        <Typography variant="h6">Scheduled Report / Segment Action</Typography>
        <Typography variant="body2" color="text.secondary" mb={2}>
          분석에서 Action까지 연결합니다: Segment → Webhook → CRM/Mail/AI
          Agent/사내 메시지.
        </Typography>
        <Stack spacing={1.5}>
          <TextField
          ...
          <TextField
            ...
            onChange={(e) =>
              setSchedule({ ...schedule, channel_id: e.target.value })
            }
            helperText={
              channels.data?.length
                ? ""
                : "먼저 Delivery Channel을 등록하십시오."
            }
          >
          ...
          <TextField
            ...
            onChange={(e) =>
              setSchedule({
                ...schedule,
                report_kind: e.target.value as ScheduleKind,
              })
            }
          >
            {(Object.keys(scheduleKindLabel) as ScheduleKind[]).map((value) => (
              <MenuItem key={value} value={value}>
                {scheduleKindLabel[value]}
              </MenuItem>
            ))}
          </TextField>
          <TextField
          ...
          >
            ...
            {!intervalPresets.some(
              (preset) => preset.minutes === schedule.interval_minutes,
            ) && (
              <MenuItem value={schedule.interval_minutes}>
          ...
          {kindUsesAlertState(schedule.report_kind) && (
            <>
              ...
              >
                {["new", "ongoing", "recovered"].map((value) => (
                  <MenuItem key={value} value={value}>
                    {alertStateLabel(value)}
                  </MenuItem>
                ))}
              </TextField>
              <FormControlLabel
                control={
                  <Checkbox
                    checked={form.alwaysSend}
                    onChange={(e) =>
                      setForm({ ...form, alwaysSend: e.target.checked })
                    }
                  />
                }
                label="이상이 없어도 매번 전송"
          ...
          {kindUsesSegmentFilters(schedule.report_kind) && (
            <>
              <TextField
                ...
                helperText="Segment 화면에서 만든 정의를 그대로 평가합니다. 고르지 않으면 아래 조건만 적용합니다."
              >
          ...
          <Button
            variant="contained"
            disabled={
              !schedule.name || !schedule.channel_id || scheduleSave.isPending
            }
            onClick={() => scheduleSave.mutate()}
          >
            Schedule 저장
          </Button>
        ...
        <Box mt={2}>
          <DataTable
            ...
            columns={[
              ...
              { key: "interval_minutes", label: "Interval", align: "right" },
                ...
                format: (v) => (
                  ...
                  >
                    <Button
                      ...
                      onClick={() => runNow.mutate(String(v))}
                    >
                      지금 실행
                    </Button>
                    <Button
      ...
      <Card sx={{ p: 2.5 }}>
        ...
        >
          <Box>
            ...
            <Typography variant="body2" color="text.secondary">
              최근 전송 결과와 응답 코드입니다. 실패는 Allowlist, 인증 Header,
              대상 시스템 순으로 확인하십시오.
            </Typography>
          </Box>
          <Button
        ...
        {runs.isLoading ? (
          ...
          <DataTable
            rows={runs.data || []}
            exportFilename="momento-delivery-runs"
            columns={[
              {
                key: "started_at",
                label: "시작",
