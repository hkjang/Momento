...
export type EnterpriseAnalyticsMode =
  | "workspace"
  | "features"
  | "search"
  | "frustration"
...
function WorkspaceRollup() {
  ...
  return (
    <Stack spacing={2}>
      {range}
      <Alert severity="info">
        동일 SSO User ID는 사이트를 넘어 한 사람으로 계산하고, 익명 Visitor는
        사이트별로 격리합니다.
      </Alert>
      <Box
        sx={{
      ...
      >
        <MetricCard
          label="등록 서비스"
          value={q.data?.summary.registered_services || 0}
        />
        <MetricCard
          label="Workspace Users"
          value={q.data?.summary.users || 0}
        />
...
type WorkspaceJourney = {
  ...
  name: string;
  description: string;
  steps: { name: string; event: string; site_id?: string }[];
...
function WorkspaceJourneys() {
  ...
  const save = useMutation({
    mutationFn: () =>
      post(`/api/v1/sites/${site!.site_id}/workspace-journeys`, {
        name,
    ...
    onSuccess: () => {
      setName("");
      void qc.invalidateQueries({ queryKey: ["workspace-journeys"] });
    },
  });
  ...
  return (
    <Card sx={{ p: 2.5 }}>
      ...
      <Typography variant="body2" color="text.secondary" mb={2}>
        서비스 경계를 넘는 업무 흐름의 단계별 전환을 SSO User 기준으로
        분석합니다. 현재 환경은 {environment.toUpperCase()}입니다.
      </Typography>
      <Box
        sx={{
          display: "grid",
      ...
      >
        ...
        <TextField
          ...
          helperText="2~12단계. name과 event는 필수이고 site_id·service·feature로 좁힐 수 있습니다."
        />
      </Box>
      <Divider sx={{ my: 2 }} />
      <Stack
      ...
      >
        ...
        <Button
          ...
          onClick={() => analysis.mutate(selected)}
        >
          분석 실행
        </Button>
      </Stack>
      ...
      {analysis.data && (
        <Box mt={2}>
          <Alert severity="info" sx={{ mb: 2 }}>
            {analysis.data.identity_policy}
          </Alert>
          <DataTable
            rows={analysis.data.steps}
            exportFilename="momento-workspace-journey"
            columns={[
              { key: "step", label: "단계", align: "right" },
              { key: "name", label: "이름" },
              { key: "event", label: "Event" },
              {
                key: "site_id",
                label: "Site",
                format: (v) => String(v || "전체"),
              },
              { key: "users", label: "Users", align: "right" },
              {
                key: "conversion_rate",
    ...
    <Stack spacing={2}>
      ...
      <DataTable
        ...
        columns={[
          ...
          { key: "feature_score", label: "Score", align: "right" },
          {
            key: "dead_feature",
            label: "판정",
            format: (v) => (
...
function SearchAnalytics() {
  ...
  const setup = searchSetupHint(
    q.data?.summary,
    q.data?.queries as { query?: unknown }[] | undefined,
  );
  ...
  return (
    <Stack spacing={2}>
      ...
      >
        <MetricCard label="Searches" value={q.data?.summary.searches || 0} />
        <MetricCard label="Search Users" value={q.data?.summary.users || 0} />
        <MetricCard
          label="Zero Result"
          value={q.data?.summary.zero_result_rate || 0}
          type="percent"
        />
        <MetricCard
          label="Search CTR"
          value={q.data?.summary.search_ctr || 0}
          type="percent"
        />
      </Box>
      {setup && <Alert severity="info">{setup}</Alert>}
      {zeroResult && <Alert severity="warning">{zeroResult}</Alert>}
      {!!q.data?.audiences?.length && (
        <Card sx={{ p: 2.5 }}>
          <AudienceList
      ...
      <DataTable
        ...
        columns={[
          ...
          { key: "searches", label: "검색", align: "right" },
          { key: "users", label: "Users", align: "right" },
          { key: "zero_results", label: "Zero Result", align: "right" },
          { key: "clicks", label: "Clicks", align: "right" },
          { key: "ctr", label: "CTR", align: "right", format: percentCell },
  ...
  return (
    <Stack spacing={2}>
      ...
      >
        <MetricCard
          label="영향 Session"
          value={q.data?.summary.affected_sessions || 0}
        />
        <MetricCard
          label="영향률"
          value={q.data?.summary.affected_session_rate || 0}
          type="percent"
        />
        <MetricCard
          label="평균 Frustration Score"
          value={q.data?.summary.average_frustration_score || 0}
        />
      </Box>
      <Alert severity="info">
        Session Replay 없이 행동 신호만 사용해 개인정보 노출을 줄입니다. Rage
        Click, Dead Click, Rapid Back, Form Retry, Repeated Search, Error After
        Click, Slow Interaction은 tracker가 자동 감지합니다.
      </Alert>
      {setup && <Alert severity="warning">{setup}</Alert>}
      {headline && (
        <Alert
          severity={
            q.data?.impact?.some((row) => row.verdict === "worse")
              ? "error"
              : "success"
          }
        >
      ...
      {!!q.data?.impact?.length && (
        <Card sx={{ p: 2.5 }}>
          <Typography variant="h6">신호별 전환 영향</Typography>
          <Typography variant="body2" color="text.secondary" mb={2}>
            {q.data.impact_caveat}
          </Typography>
          <DataTable
            exportFilename="momento-frustration-impact"
            rows={q.data.impact.map((row) => ({
              ...row,
              signal_label: describeSignal(row.signal).label,
              verdict_label: impactVerdictLabel[row.verdict],
            }))}
            columns={[
              ...
              {
                key: "affected_conversion_rate",
                label: "전환율",
                align: "right",
                format: percentCell,
              },
              {
                key: "unaffected_conversion_rate",
                label: "겪지 않은 사람 전환율",
                align: "right",
                format: percentCell,
                minWidth: 170,
              },
              {
                ...
                align: "right",
                format: (v) =>
                  `${Number(v) >= 0 ? "+" : ""}${Number(v).toFixed(1)}%p`,
              },
              {
                key: "estimated_lost_conversions",
                label: "전환 손실(추정)",
                align: "right",
                minWidth: 150,
              },
      ...
      {!!q.data?.audiences?.length && (
        <Card sx={{ p: 2.5 }}>
          <AudienceList
            audiences={q.data.audiences}
            siteId={site.site_id}
            source="Frustration 분석에서 생성"
          />
        </Card>
      )}
      <DataTable
        exportFilename="momento-frustration"
        rows={(q.data?.signals || []).map((row) => ({
          ...row,
          signal_label: describeSignal(String(row.signal)).label,
          signal_action: describeSignal(String(row.signal)).action,
        }))}
        columns={[
          {
            key: "signal_label",
            label: "Signal",
            format: (v, row) => (
              <Tooltip title={describeSignal(String(row.signal)).meaning}>
                <span>{String(v)}</span>
              </Tooltip>
            ),
          },
          { key: "signal_action", label: "확인할 것", minWidth: 260 },
          {
            key: "signal",
            label: "대상",
            format: (v) => (
              <Button
                ...
                to={`/user-explorer?q=${encodeURIComponent(String(v))}`}
              >
                겪은 사람 찾기
              </Button>
            ),
  ...
  return (
    <Stack spacing={2}>
      ...
      {analysis.isLoading ? (
        ...
        <>
          <Alert severity="info">{analysis.data.method}</Alert>
          <DataTable
            rows={analysis.data.variants}
            columns={[
              ...
              { key: "metric_value", label: "Primary Metric", align: "right" },
              {
                key: "lift_percent",
                label: "Lift",
                align: "right",
...
function Goals() {
  ...
  return (
    <Stack spacing={2}>
      <Alert severity="info">
        착지 예상치는 기간 진행률과 현재 누적 값을 연장한 추정입니다. 비율
        지표는 누적되지 않으므로 현재 관측값을 그대로 사용합니다.
      </Alert>
      <DataTable
        rows={q.data || []}
        ...
        columns={[
          ...
          { key: "period", label: "Period" },
          {
            ...
            align: "right",
            format: (v) =>
              v === undefined || v === null ? "—" : `${Number(v).toFixed(0)}%`,
          },
          {
          ...
          },
          {
            ...
            format: (v, row) =>
              ...
              v === null ? (
                <Tooltip
                  title={String(row.forecast_reason || "추정할 수 없습니다.")}
                >
                  <span>—</span>
                </Tooltip>
              ) : (
          ...
          },
          {
            ...
            format: (v, row) =>
              row.forecast_available === false ? (
                <Chip size="small" label="추정 보류" />
              ) : (
                <Chip
                  size="small"
                  color={v === "on_track" ? "success" : "error"}
                  label={v === "on_track" ? "달성 전망" : "미달 전망"}
                />
              ),
          },
          {
...
function ChangeCalendar() {
  ...
  return (
    <Stack spacing={2}>
      <Alert severity="info">
        배포·릴리즈·장애·교육·캠페인을 분석 Timeline에 함께 기록합니다.
      </Alert>
      <DataTable
        rows={q.data || []}
        columns={[
