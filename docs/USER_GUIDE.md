...
- 관리자 → Action에서 `anomaly` 종류의 Scheduled Report를 만들면 **이상이 감지된 경우에만** Webhook·Mail·사내 메시지·AI Agent로 전송합니다. 이상이 없으면 전송 이력에 `skipped`로 남습니다. `always_send: true`를 정의에 넣으면 매번 보냅니다.
...
`배분 전환`과 함께 `관여 전환`(경로에 등장한 횟수)과 `관여만`(이 모델에서 배분받지 못한 전환)을 제공해 모델 간 차이를 확인할 수 있습니다. Lookback(기본 30일) 안에 방문 기록이 없는 전환은 `미배분`으로 분리 표기합니다.
