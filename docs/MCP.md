...
- `analyze_attribution`: 단일 터치(`last_non_direct`, `first_touch`, `last_touch`)와 다중 터치(`linear`, `time_decay`, `position_based`) 모델별 채널 전환 기여도, 관여 전환, 평균 경로 길이. `half_life_days`로 시간 감쇠 반감기를 조정합니다
