...
- 7개 화면이 같은 질문에 같은 답을 하는지 21개 수치로 비교 — 전부 일치
- 픽스처 수치가 서로 같아 비교가 무의미해지는 것을 방지(구분되는 데이터 선주입 + 동일값 시 실패)
- 리포트 하나를 틀리게 만들어 가드가 걸리는지 확인
- `/api/v1/funnel` 요청 본문 문서화, 중복 키 제거, `/query`와의 날짜 범위 형태 차이 명시

## v0.33.2 — implemented

- 개인 API 키가 도달 가능한 변경 경로를 전수 실측 — 결함 없음(57개 중 51개 즉시 거부, 6개는 POST로 보내는 조회)
- 라우터를 순회하며 그 경계를 매번 확인하는 테스트 추가(누락 시 경로 이름과 함께 실패)
- v0.33.1로 사실이 아니게 된 RBAC 표 수정
- API 키의 권한 범위를 문서화(기존에 문서 없음)

## v0.33.1 — implemented

- workspace_admin이 스스로를 super_admin으로 승격할 수 있던 권한 상승 수정
- 인스턴스 전체 설정(설정·네트워크·사용자 계정)을 organization_admin 이상으로 제한
- 자기 역할 변경 금지, 자기보다 높은 역할 부여 금지(두 가드 각각 독립 검증)
- 나머지 18개 uuid 주소 경로를 실제 요청으로 전수 점검 — 모두 정상 거부
- 콘솔이 권한 없는 관리 섹션을 감추고 이유를 설명

## v0.33.0 — implemented

- 다른 Workspace의 사이트를 이름 변경·키 회전·삭제할 수 있던 권한 결함 수정(6개 경로 전부 공통 helper로 통일)
- 소유 Workspace의 관리 권한은 그대로 유지되는지 양방향 검증
- `site_id`를 가진 37개 테이블 전부가 sites에서 CASCADE되는지 검사
- 사이트 삭제 경로 전 구간 검증(확인 문자열 거부·전체 삭제·이웃 사이트 보존·감사 로그 잔존)

## v0.32.9 — implemented

- 소스를 읽는 가드 3종이 실제로 검사하는 범위를 측정 — 전부 일부만 보고 있었음
- `DELETE /api/v1/sites/{id}`(되돌릴 수 없는 삭제)가 API 계약에 없던 것 발견·문서화, 가드가 method까지 비교
- 매출 속성 가드를 28건 전수 분류로 교체(기존 3건만 검사, `internal/service` 미포함)
- 쿼리 정책 가드를 22개 리포트 전수로 확대, 새 리포트 추가 시 실패하는 검사 추가

## v0.32.8 — implemented

- 엄격한 계약(reject mode)에서 모든 세션의 첫 배치가 버려지던 결함 수정(`session_start` 미등록)
- 그것을 막으라고 있던 가드가 다른 emission 경로를 보지 않던 사각지대 제거
- 오프라인 상태의 페이지 이탈 배치를 beacon에 넘기지 않고 큐에 보존
- 큐 한도 초과 손실을 `collection_dropped`로 보고

## v0.32.7 — implemented

- 오프라인 큐 이벤트를 실제로 발생한 세션으로 전송(세션별 payload 분리)
- 중복 전송이 집계를 부풀리지 않음을 실측 확인
- 페이지 내 시간 창을 단조 시계로 측정 — 시스템 시계 역행에 영향받지 않음
- 시계 역행을 강제하는 결정적 테스트 추가, CI flake 해소(14회 연속 통과)

## v0.32.6 — implemented

- 보존 정책 화면이 직전 무인 작업을 보고(시각·소요·테이블별 삭제 수·실패 원인)
- 미실행 / 실행했으나 삭제 없음 / 실패 세 상태를 구분
- 실패 경로를 실제로 유발해 자기 실패를 기록하는지 확인
- 기록 자체를 최근 200회로 제한
- Migration 015 `retention_runs`, 업그레이드 테스트가 14개 지점에서 커버

## v0.32.5 — implemented

- 보존 삭제를 2만 행 배치로 나눠 커밋 — 중단되어도 진행분 유지, 다음 회차에 수렴
- 동시 삭제 시 짧은 배치를 완료로 오인해 조기 종료하던 버그 수정(전체 스위트 실행에서 발견)
- 배치 구조를 실제로 구분하는 가드 추가(미배치 코드에서 실패 확인)
- 관리자 가이드에 회차당 동작과 사람 단위 롤업 명시
...
- 제약 위반·NOT NULL·행 삭제 세 가지 실패 형태로 가드가 실제로 걸리는지 확인
- v0.21.2 → v0.32.2 실제 업그레이드를 수동 검증(데이터·비밀·키·43개 조회 엔드포인트)

## v0.32.2 — implemented

- 릴리즈 워크플로가 게시 전에 오프라인 아티팩트를 load·기동·검증(ready·버전·콘솔·부트스트랩 로그인)
- v0.32.1 아티팩트를 문서 절차대로 수동 검증

## v0.32.1 — implemented

- 릴리즈 누락 자동 복구 워크플로 추가(최근 7일 태그와 릴리즈 대조, 누락·불완전 시 릴리즈 워크플로 실행)
- 복구 경로를 실제로 실행해 권한과 호출을 검증
...
- 항상 비어 있던 API Key `Scope` 컬럼 제거, Key의 실제 권한 범위를 문서화
